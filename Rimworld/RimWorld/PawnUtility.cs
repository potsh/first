...
namespace RimWorld
{
	public static class PawnUtility
	{
		...
		{
			TraverseParms tp = (!passDoors) ? TraverseParms.For(pawn) : TraverseParms.For(TraverseMode.PassDoors);
		...
		public static bool ShouldSendNotificationAbout(Pawn p)
		{
			...
			if (p.IsWorldPawn() && (!p.IsCaravanMember() || !p.GetCaravan().IsPlayerControlled) && !IsTravelingInTransportPodWorldObject(p) && p.Corpse.DestroyedOrNull())
			{
				return false;
			}
				...
				for (int i = 0; i < tmpThings.Count; i++)
				{
					...
					if (pawn != null)
					{
						...
						else
						{
							...
							DamageInfo damageInfo = new DamageInfo(crush, amount, armorPenetration, -1f, null, brain, null, DamageInfo.SourceCategory.Collapse);
