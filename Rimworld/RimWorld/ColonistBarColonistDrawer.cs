...
namespace RimWorld
{
	...
	public class ColonistBarColonistDrawer
	{
		...
		public static readonly Vector3 PawnTextureCameraOffset;
		...
		public Rect GetPawnTextureRect(Vector2 pos)
		{
			...
			float y2 = vector.y;
			Vector2 size = ColonistBar.Size;
			Rect rect = new Rect(x2, num - (y2 - size.y) - 1f, vector.x, vector.y);
			return rect.ContractedBy(1f);
		}
		...
		static ColonistBarColonistDrawer()
		{
			...
			PawnTextureSize = new Vector2(baseSize.x - 2f, 75f);
			PawnTextureCameraOffset = new Vector3(0f, 0f, 0.3f);
			bracketLocs = new Vector2[4];
		}
	}
}
