...
namespace RimWorld
{
	public class RiverMaker
	{
		...
		public RiverMaker(Vector3 center, float angle, RiverDef riverDef)
		{
			...
			coordinateX = new Rotate(0.0, (double)(0f - angle), 0.0, coordinateX);
			coordinateZ = new Rotate(0.0, (double)(0f - angle), 0.0, coordinateZ);
			coordinateX = new Translate((double)(0f - center.x), 0.0, (double)(0f - center.z), coordinateX);
			coordinateZ = new Translate((double)(0f - center.x), 0.0, (double)(0f - center.z), coordinateZ);
			...
			shallowizer = new Perlin(0.029999999329447746, 2.0, 0.5, 3, Rand.Range(0, 2147483647), QualityMode.Medium);
		...
		{
			IntVec3 intVec = (from loc in lhs
			...
			IntVec3 intVec2 = (from loc in rhs
			where loc.InBounds(map) && loc.GetTerrain(map) == TerrainDefOf.WaterMovingShallow
			select loc).RandomElementWithFallback(IntVec3.Invalid);
