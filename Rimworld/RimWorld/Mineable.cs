...
namespace RimWorld
{
	public class Mineable : Building
	{
		...
		private void TrySpawnYield(Map map, float yieldChance, bool moteOnWaste, Pawn pawn)
		{
			...
			{
				int num = Mathf.Max(1, Mathf.RoundToInt((float)def.building.mineableYield * Find.Storyteller.difficulty.mineYieldFactor));
		...
		{
			int num = Mathf.Min(amount, HitPoints);
			float num2 = (float)num / (float)base.MaxHitPoints;
			yieldPct += num2 * miner.GetStatValue(StatDefOf.MiningYield);
		}
	}
}
