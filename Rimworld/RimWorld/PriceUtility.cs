...
namespace RimWorld
{
	public static class PriceUtility
	{
		...
		public static float PawnQualityPriceFactor(Pawn pawn, StringBuilder explanation = null)
		{
			...
			{
				num = (pawn.health.capacities.CapableOf(allDefsListForReading[i]) ? (num * Mathf.Lerp(0.5f, 1f, pawn.health.capacities.GetLevel(allDefsListForReading[i]))) : (num * 0.6f));
			}
			if (pawn.skills != null)
			{
				num *= AverageSkillCurve.Evaluate(pawn.skills.skills.Average((SkillRecord sk) => (float)sk.Level));
			}
				...
				if (hediffs[i] is Hediff_AddedPart && hediffs[i].def.spawnThingOnRemoved != null)
				{
					...
					if (!(baseMarketValue < 1f))
					{
						...
						if (explanation != null)
						{
							...
							explanation.Append(hediffs[i].LabelBaseCap + ": " + baseMarketValue.ToStringMoneyOffset());
						}
					}
				}
			}
