...
namespace RimWorld
{
	public class CompAnimalInsanityPulser : ThingComp
	{
		...
		{
			IEnumerable<Pawn> enumerable = from p in parent.Map.mapPawns.AllPawnsSpawned
			where p.RaceProps.Animal && p.Position.InHorDistOf(parent.Position, (float)Props.radius)
			select p;
			bool flag = false;
			...
			if (flag)
			{
				Messages.Message("MessageAnimalInsanityPulse".Translate(), parent, MessageTypeDefOf.ThreatSmall);
