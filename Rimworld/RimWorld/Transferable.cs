...
namespace RimWorld
{
	public abstract class Transferable : IExposable
	{
		...
		public int CountToTransferToSource => (PositiveCountDirection != 0) ? (-CountToTransfer) : CountToTransfer;

		public int CountToTransferToDestination => (PositiveCountDirection != 0) ? CountToTransfer : (-CountToTransfer);
		...
		public AcceptanceReport CanAdjustTo(int destination)
		{
			...
			int num = ClampAmount(destination);
