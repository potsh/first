...
namespace RimWorld
{
	public abstract class IncidentWorker_Ambush : IncidentWorker
	{
		...
		private bool DoExecute(IncidentParms parms, List<Pawn> generatedEnemies, IntVec3 existingMapEdgeCell)
		{
			...
			string letterLabel = GetLetterLabel(generatedEnemies[0], parms);
			string letterText = GetLetterText(generatedEnemies[0], parms);
			...
			Find.LetterStack.ReceiveLetter(letterLabel, letterText, GetLetterDef(generatedEnemies[0], parms), generatedEnemies[0], parms.faction);
			if (flag)
			{
