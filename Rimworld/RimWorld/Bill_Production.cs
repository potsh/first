...
namespace RimWorld
{
	public class Bill_Production : Bill, IExposable
	{
		...
		protected override float StatusLineMinHeight => (!CanUnpause()) ? 0f : 24f;
		...
		{
			return repeatMode == BillRepeatModeDefOf.TargetCount && paused && pauseWhenSatisfied && recipe.WorkerCounter.CountProducts(this) < targetCount;
		}
		...
		public override void DoStatusLineInterface(Rect rect)
		{
			if (paused)
			{
				WidgetRow widgetRow = new WidgetRow(rect.xMax, rect.y, UIDirection.LeftThenUp);
				if (widgetRow.ButtonText("Unpause".Translate()))
				{
					paused = false;
				}
			}
		...
		public override Bill Clone()
		{
			...
			bill_Production.repeatMode = repeatMode;
			bill_Production.repeatCount = repeatCount;
			bill_Production.storeMode = storeMode;
			bill_Production.storeZone = storeZone;
			bill_Production.targetCount = targetCount;
			bill_Production.pauseWhenSatisfied = pauseWhenSatisfied;
			bill_Production.unpauseWhenYouHave = unpauseWhenYouHave;
			bill_Production.includeEquipped = includeEquipped;
			bill_Production.includeTainted = includeTainted;
			bill_Production.includeFromZone = includeFromZone;
			bill_Production.hpRange = hpRange;
			bill_Production.qualityRange = qualityRange;
			bill_Production.limitToAllowedStuff = limitToAllowedStuff;
			bill_Production.paused = paused;
