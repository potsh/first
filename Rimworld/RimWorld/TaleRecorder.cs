...
namespace RimWorld
{
	public static class TaleRecorder
	{
		...
		public static Tale RecordTale(TaleDef def, params object[] args)
		{
			...
			{
				Log.Message(string.Format("Tale {0} from {1}, targets {2}:\n{3}", (!flag) ? "recorded" : "ignored", def, args.Select(Gen.ToStringSafe<object>).ToCommaList(), TaleTextGenerator.GenerateTextFromTale(TextGenerationPurpose.ArtDescription, tale, Rand.Int, RulePackDefOf.ArtDescription_Sculpture)));
			}
			if (flag)
			{
				return null;
