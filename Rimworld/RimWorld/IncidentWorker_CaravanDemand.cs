...
namespace RimWorld
{
	public class IncidentWorker_CaravanDemand : IncidentWorker
	{
		...
		{
			Faction faction;
			return CaravanIncidentUtility.CanFireIncidentWhichWantsToGenerateMapAt(parms.target.Tile) && PawnGroupMakerUtility.TryGetRandomFactionForCombatPawnGroup(parms.points, out faction);
		}
		...
		private List<ThingCount> TryGenerateColonistOrPrisonerDemand(Caravan caravan)
		{
			...
			if (list.Any())
			{
				...
				list2.Add(new ThingCount(list.RandomElement(), 1));
				return list2;
			}
			return null;
		}
