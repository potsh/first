			...
			for (int j = 0; j < 2; j++)
			{
				...
				if (flag)
				{
					for (int k = 0; k < endcapSamples.Length; k++)
					{
						if (array[k] != 0f)
						{
							int num5 = Mathf.RoundToInt((float)input.Count * endcapSamples[k]);
							...
							PawnPath pawnPath2 = map.pathFinder.FindPath(input[num5], input[input.Count - 1], TraverseParms.For(TraverseMode.NoPassClosedDoors));
							if (pawnPath2 == PawnPath.NotFound)
							{
								pawnPath2 = map.pathFinder.FindPath(input[num5], input[input.Count - 1], TraverseParms.For(TraverseMode.PassAllDestroyableThings));
							}
							if (pawnPath2 != PawnPath.NotFound)
							{
