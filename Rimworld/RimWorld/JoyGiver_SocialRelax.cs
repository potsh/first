...
namespace RimWorld
{
	public class JoyGiver_SocialRelax : JoyGiver
	{
		...
		{
			return TryGiveJobInt(pawn, (CompGatherSpot x) => PartyUtility.InPartyArea(x.parent.Position, partySpot, pawn.Map));
		}
		...
		private Job TryGiveJobInt(Pawn pawn, Predicate<CompGatherSpot> gatherSpotValidator)
		{
			...
			if (result.parent.def.surfaceType == SurfaceType.Eat)
			{
				...
				job = new Job(def.jobDef, result.parent, chair);
			}
			else if (TryFindChairNear(result.parent.Position, pawn, out chair2))
			{
				job = new Job(def.jobDef, result.parent, chair2);
			}
			else
			{
				...
				job = new Job(def.jobDef, result.parent, result2);
			}
