...
namespace RimWorld
{
	public class Filth : Thing
	{
		...
		public bool CanFilthAttachNow => def.filth.canFilthAttach && thickness > 1 && Find.TickManager.TicksGame - growTick > 400;
		...
		public override string Label
		{
			get
			{
				...
				if (!sources.NullOrEmpty())
				{
					...
					stringBuilder.Append(sources.ToCommaList(useAnd: true));
				}
		...
		public bool CanDropAt(IntVec3 c, Map map)
		{
			...
			if (def.filth.terrainSourced && !terrainDef.acceptTerrainSourceFilth)
			{
				return false;
			}
			return true;
