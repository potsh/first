...
namespace RimWorld
{
	public class InspirationHandler : IExposable
	{
		...
		public InspirationDef CurStateDef => (curState == null) ? null : curState.def;
		...
		private InspirationDef GetRandomAvailableInspirationDef()
		{
			...
			select x).RandomElementByWeightWithFallback((InspirationDef x) => x.Worker.CommonalityFor(pawn));
		}
	}
}
