...
namespace RimWorld
{
	public class SteadyEnvironmentEffects
	{
				...
				if (!flag2)
				{
					...
					if (temperature > 0f)
					{
						...
						if (room.RegionType.Passable())
						{
							...
							FloatRange autoIgnitionTemperatureRange = AutoIgnitionTemperatureRange;
		...
		private void TryDoDeteriorate(Thing t, bool roofed, bool roomUsesOutdoorTemperature, bool protectedByEdifice, TerrainDef terrain)
		{
			...
			if (!(num < 0.001f))
			{
				float chance = deteriorationRate * num / 36f;
		...
		{
			float chance = 0.2f * (float)map.listerBuildings.allBuildingsColonistElecFire.Count * map.weatherManager.RainRate;
			if (Rand.Chance(chance))
			{
