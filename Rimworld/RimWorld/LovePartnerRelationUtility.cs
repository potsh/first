...
namespace RimWorld
{
	public static class LovePartnerRelationUtility
	{
		...
		{
			return pawn.relations.GetFirstDirectRelationPawn(PawnRelationDefOf.Spouse) != null || pawn.relations.GetFirstDirectRelationPawn(PawnRelationDefOf.Lover) != null || pawn.relations.GetFirstDirectRelationPawn(PawnRelationDefOf.Fiance) != null;
		}
		...
		public static Pawn GetMostDislikedNonPartnerBedOwner(Pawn p)
		{
			...
			{
				if (ownedBed.owners[i] != p && !LovePartnerRelationExists(p, ownedBed.owners[i]))
				{
