	...
	public static class WatchBuildingUtility
	{
		...
		public static IEnumerable<IntVec3> CalculateWatchCells(ThingDef def, IntVec3 center, Rot4 rot, Map map)
		{
			...
			for (int i = 0; i < allowedDirections.Count; i++)
			{
				...
				{
					if (EverPossibleToWatchFrom(item, center, map, bedAllowed: true))
					{
						yield return item;
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					if (watchCellRect.Contains(intVec2))
					{
						...
						if (EverPossibleToWatchFrom(intVec2, toWatch.Position, toWatch.Map, bedAllowed: false) && !intVec2.IsForbidden(pawn) && pawn.CanReserve(intVec2) && pawn.Map.pawnDestinationReservationManager.CanReserve(intVec2, pawn))
						{
							if (desireSit)
							{
		...
		{
			return (watchCell.Standable(map) || (bedAllowed && watchCell.GetEdifice(map) is Building_Bed)) && GenSight.LineOfSight(buildingCenter, watchCell, map, skipFirstCell: true);
		}
