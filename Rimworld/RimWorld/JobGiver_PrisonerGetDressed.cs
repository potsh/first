	...
	public class JobGiver_PrisonerGetDressed : ThinkNode_JobGiver
	{
		protected override Job TryGiveJob(Pawn pawn)
		{
			if (pawn.guest.PrisonerIsSecure && pawn.apparel != null)
			{
				...
				{
					Apparel apparel = FindGarmentCoveringPart(pawn, BodyPartGroupDefOf.Legs);
					if (apparel != null)
					{
				...
				{
					Apparel apparel2 = FindGarmentCoveringPart(pawn, BodyPartGroupDefOf.Torso);
					if (apparel2 != null)
					{
			...
			if (room.isPrisonCell)
			{
				foreach (IntVec3 cell in room.Cells)
				{
					...
					for (int i = 0; i < thingList.Count; i++)
					{
						...
						if (apparel != null && apparel.def.apparel.bodyPartGroups.Contains(bodyPartGroupDef) && pawn.CanReserve(apparel) && !apparel.IsBurning() && ApparelUtility.HasPartsToWear(pawn, apparel.def))
						{
							return apparel;
						}
					}
