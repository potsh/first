...
namespace RimWorld
{
	public static class PawnBanishUtility
	{
		...
		public static void Banish(Pawn pawn, int tile = -1)
		{
			...
			else
			{
				...
				if (pawn.Faction == Faction.OfPlayer)
				{
					...
					if (!pawn.Spawned && Find.FactionManager.TryGetRandomNonColonyHumanlikeFaction(out faction, pawn.Faction != null && (int)pawn.Faction.def.techLevel >= 3))
					{
		...
		public static string GetBanishPawnDialogText(Pawn banishedPawn)
		{
			...
			ThingOwner<Thing> thingOwner = (banishedPawn.inventory == null || !WillTakeInventoryIfBanished(banishedPawn)) ? null : banishedPawn.inventory.innerContainer;
			if (!list.NullOrEmpty() || !list2.NullOrEmpty() || !thingOwner.NullOrEmpty())
			{
				stringBuilder.AppendLine();
				stringBuilder.AppendLine();
				stringBuilder.Append("ConfirmBanishPawnDialog_Items".Translate(banishedPawn.LabelShort, banishedPawn).CapitalizeFirst().AdjustedFor(banishedPawn));
				stringBuilder.AppendLine();
				if (list != null)
				{
