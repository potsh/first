	...
	public class InspirationWorker
	{
		...
		public virtual bool InspirationCanOccur(Pawn pawn)
		{
			...
			if (def.requiredNonDisabledWorkTypes != null)
			{
				...
				{
					if (pawn.story == null || pawn.story.WorkTypeIsDisabled(def.requiredNonDisabledWorkTypes[l]))
					{
						return false;
					}
				}
			...
			if (!def.requiredAnyNonDisabledWorkType.NullOrEmpty())
			{
				...
				{
					if (pawn.story != null && !pawn.story.WorkTypeIsDisabled(def.requiredAnyNonDisabledWorkType[m]))
					{
						flag2 = true;
						break;
					}
