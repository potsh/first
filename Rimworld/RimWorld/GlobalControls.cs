	...
	public class GlobalControls
	{
		...
		private static string TemperatureString()
		{
			...
			if (room == null && intVec.InBounds(Find.CurrentMap))
			{
				...
				if (edifice != null)
				{
					CellRect.CellRectIterator iterator = edifice.OccupiedRect().ExpandedBy(1).ClipInsideMap(Find.CurrentMap)
						.GetIterator();
					while (!iterator.Done())
					{
			...
			string str = (!c.InBounds(Find.CurrentMap) || c.Fogged(Find.CurrentMap) || room == null || room.PsychologicallyOutdoors) ? "Outdoors".Translate() : ((room.OpenRoofCount != 0) ? ("IndoorsUnroofed".Translate() + " (" + room.OpenRoofCount.ToStringCached() + ")") : "Indoors".Translate());
