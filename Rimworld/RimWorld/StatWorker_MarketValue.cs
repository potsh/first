...
namespace RimWorld
{
	public class StatWorker_MarketValue : StatWorker
	{
		...
		public override float GetValueUnfinalized(StatRequest req, bool applyPostProcess = true)
		{
			...
			{
				return base.GetValueUnfinalized(StatRequest.For(req.Def, req.StuffDef), applyPostProcess) * PriceUtility.PawnQualityPriceFactor((Pawn)req.Thing) + PriceUtility.PawnQualityPriceOffset((Pawn)req.Thing);
			}
			if (req.Def.statBases.StatListContains(StatDefOf.MarketValue))
			{
				return base.GetValueUnfinalized(req);
			}
			return CalculatedBaseMarketValue(req.Def, req.StuffDef);
		}
		...
		public override string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
		{
			...
			return "StatsReport_MarketValueFromStuffsAndWork".Translate().TrimEnd('.') + ": " + CalculatedBaseMarketValue(req.Def, req.StuffDef).ToStringByStyle(stat.ToStringStyleUnfinalized, numberSense);
		}
