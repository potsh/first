...
namespace RimWorld
{
	public class PawnColumnWorker_WorkPriority : PawnColumnWorker
	{
		...
		public override void DoHeader(Rect rect, PawnTable table)
		{
			...
			Widgets.DrawLineVertical(center.x, labelRect.yMax - 3f, rect.y + 50f - labelRect.yMax + 3f);
			Vector2 center2 = labelRect.center;
			Widgets.DrawLineVertical(center2.x + 1f, labelRect.yMax - 3f, rect.y + 50f - labelRect.yMax + 3f);
		...
		private float GetValueToCompare(Pawn pawn)
		{
			...
			if (pawn.story != null && pawn.story.WorkTypeIsDisabled(def.workType))
			{
				return -1f;
			}
		...
		protected override void HeaderClicked(Rect headerRect, PawnTable table)
		{
			...
			if (Event.current.shift)
			{
				...
				for (int i = 0; i < pawnsListForReading.Count; i++)
				{
					...
					if (pawn.workSettings != null && pawn.workSettings.EverWork && (pawn.story == null || !pawn.story.WorkTypeIsDisabled(def.workType)))
					{
						if (Find.PlaySettings.useWorkPriorities)
						{
