	...
	public class PlaceWorker_NeverAdjacentTrap : PlaceWorker
	{
		...
		public override AcceptanceReport AllowsPlacing(BuildableDef def, IntVec3 center, Rot4 rot, Map map, Thing thingToIgnore = null)
		{
			...
			while (!iterator.Done())
			{
				...
				{
					Thing thing = list[i];
					if (thing != thingToIgnore && ((thing.def.category == ThingCategory.Building && thing.def.building.isTrap) || ((thing.def.IsBlueprint || thing.def.IsFrame) && thing.def.entityDefToBuild is ThingDef && ((ThingDef)thing.def.entityDefToBuild).building.isTrap)))
					{
