	...
	public class WildPlantSpawner : IExposable
	{
		...
		public float CurrentWholeMapNumDesiredPlants
		{
			get
			{
				...
				while (!iterator.Done())
				{
					num += GetDesiredPlantsCountAt(iterator.Current, iterator.Current, currentPlantDensity);
					iterator.MoveNext();
				}
				return num;
			}
		...
		public int CurrentWholeMapNumNonZeroFertilityCells
		{
			get
			{
				...
				while (!iterator.Done())
				{
					if (iterator.Current.GetTerrain(map).fertility > 0f)
					{
						num++;
					}
					iterator.MoveNext();
		...
		public float CavePlantsCommonalitiesSum
		{
			get
			{
				float? num = cachedCavePlantsCommonalitiesSum;
				if (!num.HasValue)
				{
					cachedCavePlantsCommonalitiesSum = 0f;
					for (int i = 0; i < allCavePlants.Count; i++)
					{
						float? num2 = cachedCavePlantsCommonalitiesSum;
						cachedCavePlantsCommonalitiesSum = ((!num2.HasValue) ? null : new float?(num2.GetValueOrDefault() + GetCommonalityOfPlant(allCavePlants[i])));
					}
				}
		...
		{
			allCavePlants.Clear();
			allCavePlants.AddRange(from x in DefDatabase<ThingDef>.AllDefsListForReading
		...
		private void WildPlantSpawnerTickInternal()
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				float mtb = (!GoodRoofForCavePlant(intVec)) ? map.Biome.wildPlantRegrowDays : 130f;
				if (Rand.Chance(chance) && Rand.MTBEventOccurs(mtb, 60000f, (float)num2) && CanRegrowAt(intVec))
				{
		...
		private float PlantChoiceWeight(ThingDef plantDef, IntVec3 c, Dictionary<ThingDef, float> distanceSqToNearbyClusters, float wholeMapNumDesiredPlants, float plantDensity)
		{
			...
			{
				float num3 = (!plantDef.plant.cavePlant) ? map.Biome.PlantCommonalitiesSum : CavePlantsCommonalitiesSum;
				...
				float outTo = 1f / (3.14159274f * (float)plantDef.plant.wildClusterRadius * (float)plantDef.plant.wildClusterRadius);
			...
			if (plantDef.plant.wildEqualLocalDistribution)
			{
				...
				float num4 = (float)Mathf.Max(x3, size2.z) / Mathf.Sqrt(f);
				float a = num4 * 2f;
		...
		private bool SaturatedAt(IntVec3 c, float plantDensity, bool cavePlants, float wholeMapNumDesiredPlants)
		{
			...
			float num2 = wholeMapNumDesiredPlants * ((float)num / (float)map.Area);
			if (num2 <= 4f || !map.Biome.wildPlantsCareAboutLocalFertility)
			{
		...
		private void CalculateDistancesToNearbyClusters(IntVec3 c)
		{
			...
			int num = GenRadial.NumCellsInRadius((float)(map.Biome.MaxWildAndCavePlantsClusterRadius * 2));
			for (int i = 0; i < num; i++)
			{
		...
		{
			return c.GetTemperature(map) > 0f && (!c.Roofed(map) || GoodRoofForCavePlant(c));
		}
		...
		{
			return (!plant.plant.cavePlant) ? map.Biome.CommonalityOfPlant(plant) : plant.plant.cavePlantWeight;
		}

		private float GetCommonalityPctOfPlant(ThingDef plant)
		{
			return (!plant.plant.cavePlant) ? map.Biome.CommonalityPctOfPlant(plant) : (GetCommonalityOfPlant(plant) / CavePlantsCommonalitiesSum);
		}
		...
		{
			return Mathf.Min(reg.GetBaseDesiredPlantsCount() * plantDensity * forCell.GetTerrain(map).fertility, (float)reg.CellCount);
		}
	}
}
