...
namespace RimWorld
{
	public class SkillRecord : IExposable
	{
		...
		{
			float num = (!pawn.story.traits.HasTrait(TraitDefOf.GreatMemory)) ? 1f : 0.5f;
			switch (levelInt)
			{
			case 10:
				Learn(-0.1f * num);
		...
		{
			return XpForLevelUpCurve.Evaluate((float)startingLevel);
		}
		...
		{
			return def.IsDisabled(pawn.story.CombinedDisabledWorkTags, pawn.story.DisabledWorkTypes);
		}
