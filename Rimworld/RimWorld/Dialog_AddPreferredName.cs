...
namespace RimWorld
{
	public class Dialog_AddPreferredName : Window
	{
		...
		public Dialog_AddPreferredName()
		{
			...
			cachedNames = (from n in (from b in SolidBioDatabase.allBios
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			{
				searchName = text;
				searchWords = searchName.Replace("'", string.Empty).Split(' ');
			}
			...
			if (searchName.Length > 1)
			{
				foreach (NameTriple item in cachedNames.Where(FilterMatch))
				{
					...
					float num = listing_Standard.CurHeight + 30f;
					float height = inRect.height;
					Vector2 closeButSize = CloseButSize;
					if (num > height - (closeButSize.y + 8f))
					{
						break;
					}
				}
		...
		private bool FilterMatch(NameTriple n)
		{
			...
			{
				return n.Last.StartsWith(searchName, StringComparison.OrdinalIgnoreCase) || n.First.StartsWith(searchName, StringComparison.OrdinalIgnoreCase) || n.Nick.StartsWith(searchName, StringComparison.OrdinalIgnoreCase);
			}
			if (searchWords.Length == 2)
			{
				return n.First.EqualsIgnoreCase(searchWords[0]) && (n.Last.StartsWith(searchWords[1], StringComparison.OrdinalIgnoreCase) || n.Nick.StartsWith(searchWords[1], StringComparison.OrdinalIgnoreCase));
			}
			return false;
		}
