...
namespace RimWorld
{
	public abstract class CompTargetable : CompUseEffect
	{
		...
		private CompProperties_Targetable Props => (CompProperties_Targetable)props;
		...
		public override bool SelectedUseOption(Pawn p)
		{
			if (PlayerChoosesTarget)
			{
				...
				{
					target = t.Thing;
					parent.GetComp<CompUsable>().TryStartUseJob(p);
				}, p);
				return true;
			}
			target = null;
		...
		{
			if (Props.psychicSensitiveTargetsOnly)
			{
				Pawn pawn = t as Pawn;
				if (pawn != null && pawn.GetStatValue(StatDefOf.PsychicSensitivity) <= 0f)
				{
					return false;
				}
			}
