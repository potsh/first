	...
	public class FoodRestrictionDatabase : IExposable
	{
		...
		private void GenerateStartingFoodRestrictions()
		{
			...
			foreach (ThingDef allDef in DefDatabase<ThingDef>.AllDefs)
			{
				...
				{
					foodRestriction2.filter.SetAllow(allDef, allow: false);
				}
			}
			...
			foreach (ThingDef allDef2 in DefDatabase<ThingDef>.AllDefs)
			{
				...
				{
					foodRestriction3.filter.SetAllow(allDef2, allow: false);
				}
			}
			foodRestriction3.filter.SetAllow(ThingDefOf.MealSurvivalPack, allow: false);
			...
			foreach (ThingDef allDef3 in DefDatabase<ThingDef>.AllDefs)
			{
				...
				{
					foodRestriction4.filter.SetAllow(allDef3, allow: false);
				}
			}
			...
			foreach (ThingDef allDef4 in DefDatabase<ThingDef>.AllDefs)
			{
				...
				{
					foodRestriction5.filter.SetAllow(allDef4, allow: false);
				}
			}
			foodRestriction5.filter.SetAllow(ThingDefOf.Chocolate, allow: false);
			FoodRestriction foodRestriction6 = MakeNewFoodRestriction();
			foodRestriction6.label = "FoodRestrictionNothing".Translate();
			foodRestriction6.filter.SetDisallowAll();
		}
	}
}
