...
namespace RimWorld
{
	public class PawnBreathMoteMaker
	{
		...
		{
			Vector3 loc = pawn.Drawer.DrawPos + pawn.Drawer.renderer.BaseHeadOffsetAt(pawn.Rotation) + pawn.Rotation.FacingCell.ToVector3() * 0.21f + BreathOffset;
			Vector3 lastTickTweenedVelocity = pawn.Drawer.tweener.LastTickTweenedVelocity;
			MoteMaker.ThrowBreathPuff(loc, pawn.Map, pawn.Rotation.AsAngle, lastTickTweenedVelocity);
		}
	}
}
