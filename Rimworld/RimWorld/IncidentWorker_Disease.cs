...
namespace RimWorld
{
	public abstract class IncidentWorker_Disease : IncidentWorker
	{
		...
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
			...
			string text = (!list.Any()) ? string.Empty : string.Format(def.letterText, list.Count.ToString(), Faction.OfPlayer.def.pawnsPlural, def.diseaseIncident.label, stringBuilder.ToString());
			...
			Find.LetterStack.ReceiveLetter(def.letterLabel, text, def.letterDef, list);
			return true;
		}
