...
namespace RimWorld
{
	public static class TutorUtility
	{
		...
		public static CellRect FindUsableRect(int width, int height, Map map, float minFertility = 0f, bool noItems = false)
		{
			...
			while (true)
			{
				IntVec3 center2 = center + new IntVec3((int)Rand.Range(0f - num, num), 0, (int)Rand.Range(0f - num, num));
				...
				while (!iterator.Done())
				{
					...
					if (current.Fogged(map) || !current.Walkable(map) || !current.GetTerrain(map).affordances.Contains(TerrainAffordanceDefOf.Heavy) || current.GetTerrain(map).fertility < minFertility || current.GetZone(map) != null || ContainsBlockingThing(current, map, noItems) || current.InNoBuildEdgeArea(map) || current.InNoZoneEdgeArea(map))
					{
						flag = false;
						break;
					}
		...
		public static void DrawCellRectOnGUI(CellRect cellRect, string label = null)
		{
			if (label != null)
			{
				...
				DrawLabelOnGUI(centerVector, label);
			}
		}
		...
		public static void DoModalDialogIfNotKnown(ConceptDef conc)
		{
			if (!PlayerKnowledgeDatabase.IsComplete(conc))
			{
				...
				Find.WindowStack.Add(new Dialog_MessageBox(helpTextAdjusted));
