...
namespace RimWorld
{
	public class StoryWatcher_Adaptation : IExposable
	{
		...
		private void ResolvePawnEvent(Pawn p, AdaptationEvent ev)
		{
			...
			if (ev == AdaptationEvent.Downed)
			{
				num = StorytellerDef.adaptDaysLossFromColonistViolentlyDownedByPopulation.Evaluate((float)Population);
			}
			else
			{
				...
				num = StorytellerDef.adaptDaysLossFromColonistLostByPostPopulation.Evaluate((float)num2);
			}
