...
namespace RimWorld
{
	public class Pawn_InteractionsTracker : IExposable
	{
		...
		public void InteractionsTrackerTick()
		{
			...
			if (!wantsRandomInteract)
			{
				if (Find.TickManager.TicksGame > lastInteractionTime + 320 && pawn.IsHashIntervalTick(60))
				{
					...
					if (Rand.MTBEventOccurs((float)num, 1f, 60f) && !TryInteractRandomly())
					{
		...
		public bool CanInteractNowWith(Pawn recipient)
		{
			...
			if (!InteractionUtility.CanInitiateInteraction(pawn) || !InteractionUtility.CanReceiveInteraction(recipient))
			{
				return false;
			}
			return true;
		...
		public bool TryInteractWith(Pawn recipient, InteractionDef intDef)
		{
			...
			if (!CanInteractNowWith(recipient))
			{
				return false;
			}
			...
			{
				pawn.skills.Learn(intDef.initiatorXpGainSkill, (float)intDef.initiatorXpGainAmount);
			}
			...
			if (!flag)
			{
				intDef.Worker.Interacted(pawn, recipient, list, out letterText, out letterLabel, out letterDef);
			}
			else
			{
				letterText = null;
			...
			if (letterDef != null)
			{
				...
				Find.LetterStack.ReceiveLetter(letterLabel, text, letterDef, pawn);
			}
			return true;
		}
		...
		private bool TryInteractRandomly()
		{
			...
			for (int i = 0; i < workingList.Count; i++)
			{
				...
				if (p != pawn && CanInteractNowWith(p) && InteractionUtility.CanReceiveRandomInteraction(p) && !pawn.HostileTo(p) && allDefsListForReading.TryRandomElementByWeight((InteractionDef x) => x.Worker.RandomSelectionWeight(pawn, p), out InteractionDef result))
				{
		...
		public float SocialFightChance(InteractionDef interaction, Pawn initiator)
		{
			...
			float num = (float)pawn.relations.OpinionOf(initiator);
