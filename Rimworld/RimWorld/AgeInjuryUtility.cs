		...
		public static void GenerateRandomOldAgeInjuries(Pawn pawn, bool tryNotToKillPawn)
		{
			...
			for (int i = 0; i < num3; i++)
			{
				...
				if (source.Any())
				{
					...
					DamageDef dam = RandomPermanentInjuryDamageType(bodyPartRecord.def.frostbiteVulnerability > 0f && pawn.RaceProps.ToolUser);
			...
			for (int j = 1; j < pawn.ageTracker.AgeBiologicalYears; j++)
			{
				...
				{
					item.TryApplyAndSimulateSeverityChange(pawn, (float)j, tryNotToKillPawn);
					if (pawn.Dead)
					{
						break;
					}
