...
namespace RimWorld
{
	public class Fire : AttachableThing, ISizeReporter
	{
		...
		{
			float value = 0.0125f + 0.0036f * fireSize;
			value = Mathf.Clamp(value, 0.0125f, 0.05f);
			int num = GenMath.RoundRandom(value * 150f);
			if (num < 1)
			{
				num = 1;
			}
			...
			if (pawn != null)
			{
				...
				DamageInfo dinfo = new DamageInfo(DamageDefOf.Flame, (float)num, 0f, -1f, this);
				...
				{
					result.TakeDamage(new DamageInfo(DamageDefOf.Flame, (float)num, 0f, -1f, this));
				}
			}
			else
			{
				targ.TakeDamage(new DamageInfo(DamageDefOf.Flame, (float)num, 0f, -1f, this));
			}
		}
