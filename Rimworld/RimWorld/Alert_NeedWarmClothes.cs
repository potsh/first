...
namespace RimWorld
{
	public class Alert_NeedWarmClothes : Alert
	{
		...
		private int FreeWarmClothesSetsCount(Map map)
		{
			...
			jackets.SortBy((Thing x) => x.GetStatValue(StatDefOf.Insulation_Cold));
			shirts.SortBy((Thing x) => x.GetStatValue(StatDefOf.Insulation_Cold));
			pants.SortBy((Thing x) => x.GetStatValue(StatDefOf.Insulation_Cold));
