...
namespace RimWorld
{
	...
	public class Tornado : ThingWithComps
	{
		...
		public override void Tick()
		{
			if (base.Spawned)
			{
				...
				else
				{
					...
					direction += (float)directionNoise.GetValue((double)Find.TickManager.TicksAbs, (double)((float)(thingIDNumber % 500) * 1000f), 0.0) * 0.78f;
		...
		private void DrawTornadoPart(float distanceFromCenter, float initialAngle, float speedMultiplier, float colorMultiplier)
		{
			...
			Vector3 a = new Vector3(vector.x, AltitudeLayer.Weather.AltitudeFor() + 0.046875f * Rand.Range(0f, 1f), vector.y);
			...
			FloatRange partsDistanceFromCenter = PartsDistanceFromCenter;
