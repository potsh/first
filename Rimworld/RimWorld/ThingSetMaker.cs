...
namespace RimWorld
{
	public abstract class ThingSetMaker
	{
		...
		public IEnumerable<ThingDef> AllGeneratableThingsDebug(ThingSetMakerParams parms)
		{
			...
			{
				ThingSetMakerParams parmsToUse = ApplyFixedParams(parms);
				using (IEnumerator<ThingDef> enumerator = AllGeneratableThingsDebugSub(parmsToUse).Distinct().GetEnumerator())
				{
