...
namespace RimWorld
{
	public static class LightningBoltMeshMaker
	{
		...
		{
			int num = (int)Math.Ceiling((double)((Vector2.zero - lightningTop).magnitude / 0.25f));
		...
		private static Mesh MeshFromVerts()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				Vector2 vector = verts2D[i];
				float x = vector.x;
				Vector2 vector2 = verts2D[i];
				reference = new Vector3(x, 0f, vector2.y);
			}
			float num = 0f;
