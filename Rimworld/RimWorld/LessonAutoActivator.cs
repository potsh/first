...
namespace RimWorld
{
	public static class LessonAutoActivator
	{
		...
		private static float GetDesire(ConceptDef conc)
		{
			...
			float num = 0f;
			num += conc.priority;
			num += GetOpportunity(conc) / 100f * 60f;
		...
		public static string DebugString()
		{
			...
			foreach (ConceptDef item in DefDatabase<ConceptDef>.AllDefs.OrderByDescending(GetDesire))
			{
		...
		{
			TryInitiateLesson(DefDatabase<ConceptDef>.AllDefs.OrderByDescending(GetDesire).First());
		}
	}
}
