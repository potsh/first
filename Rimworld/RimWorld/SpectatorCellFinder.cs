	...
	public static class SpectatorCellFinder
	{
			...
			Predicate<IntVec3> predicate = delegate(IntVec3 x)
			{
				...
				if (!CorrectlyRotatedChairAt(x, map, spectateRect))
				{
					...
					for (int k = 0; k < GenAdj.AdjacentCells.Length; k++)
					{
						...
						if (CorrectlyRotatedChairAt(x2, map, spectateRect))
						{
							num++;
						}
					}
		...
		public static SpectateRectSide FindSingleBestSide(CellRect spectateRect, Map map, SpectateRectSide allowedSides = SpectateRectSide.All, int margin = 1)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (cell.z > cellRect.maxZ && (allowedSides & SpectateRectSide.Up) == SpectateRectSide.Up)
				{
					scorePerSide[0] += num3;
					if (correctlyRotatedChairAt != null && correctlyRotatedChairAt.Rotation == Rot4.South)
					{
						scorePerSide[0] += 1.2f * num2;
					}
				}
				if (cell.x > cellRect.maxX && (allowedSides & SpectateRectSide.Right) == SpectateRectSide.Right)
				{
					scorePerSide[1] += num3;
					if (correctlyRotatedChairAt != null && correctlyRotatedChairAt.Rotation == Rot4.West)
					{
						scorePerSide[1] += 1.2f * num2;
					}
				}
				if (cell.z < cellRect.minZ && (allowedSides & SpectateRectSide.Down) == SpectateRectSide.Down)
				{
					scorePerSide[2] += num3;
					if (correctlyRotatedChairAt != null && correctlyRotatedChairAt.Rotation == Rot4.North)
					{
						scorePerSide[2] += 1.2f * num2;
					}
				}
				if (cell.x < cellRect.minX && (allowedSides & SpectateRectSide.Left) == SpectateRectSide.Left)
				{
					scorePerSide[3] += num3;
					if (correctlyRotatedChairAt != null && correctlyRotatedChairAt.Rotation == Rot4.East)
					{
						scorePerSide[3] += 1.2f * num2;
					}
				}
			}
			float num4 = 0f;
