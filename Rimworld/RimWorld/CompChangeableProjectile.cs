...
namespace RimWorld
{
	public class CompChangeableProjectile : ThingComp, IStoreSettingsParent
	{
		...
		public ThingDef LoadedShell => (loadedCount <= 0) ? null : loadedShell;

		public ThingDef Projectile => (!Loaded) ? null : LoadedShell.projectileWhenLoaded;
		...
		public void LoadShell(ThingDef shell, int count)
		{
			...
			loadedShell = ((count <= 0) ? null : shell);
		}
