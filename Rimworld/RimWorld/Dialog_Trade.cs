	...
	public class Dialog_Trade : Window
	{
		...
		private float TilesPerDay
		{
			get
			{
				if (tilesPerDayDirty)
				{
					...
					StringBuilder stringBuilder = new StringBuilder();
					cachedTilesPerDay = TilesPerDayCalculator.ApproxTilesPerDayLeftAfterTradeableTransfer(playerCaravanAllPawnsAndItems, cachedTradeables, MassUsage, MassCapacity, Tile, (caravan == null || !caravan.pather.Moving) ? (-1) : caravan.pather.nextTile, stringBuilder);
		...
		public override void PostOpen()
		{
			...
			if (!giftsOnly)
			{
				...
				{
					string str = (!(level < 0.95f)) ? "NegotiatorHearingImpaired".Translate(playerNegotiator.LabelShort, playerNegotiator) : "NegotiatorTalkingImpaired".Translate(playerNegotiator.LabelShort, playerNegotiator);
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (cachedCurrencyTradeable != null)
			{
				...
				TradeUI.DrawTradeableRow(rect6, cachedCurrencyTradeable, 1);
			...
			float num4 = inRect.width / 2f;
			Vector2 acceptButtonSize = AcceptButtonSize;
			float x2 = num4 - acceptButtonSize.x / 2f;
			float y = inRect.height - 55f;
			Vector2 acceptButtonSize2 = AcceptButtonSize;
			float x3 = acceptButtonSize2.x;
			Vector2 acceptButtonSize3 = AcceptButtonSize;
			Rect rect7 = new Rect(x2, y, x3, acceptButtonSize3.y);
			if (Widgets.ButtonText(rect7, (!TradeSession.giftMode) ? "AcceptButton".Translate() : ("OfferGifts".Translate() + " (" + FactionGiftUtility.GetGoodwillChange(TradeSession.deal.AllTradeables, TradeSession.trader.Faction).ToStringWithSign() + ")")))
			{
			...
			float num5 = rect7.x - 10f;
			Vector2 otherBottomButtonSize = OtherBottomButtonSize;
			float x4 = num5 - otherBottomButtonSize.x;
			float y2 = rect7.y;
			Vector2 otherBottomButtonSize2 = OtherBottomButtonSize;
			float x5 = otherBottomButtonSize2.x;
			Vector2 otherBottomButtonSize3 = OtherBottomButtonSize;
			Rect rect8 = new Rect(x4, y2, x5, otherBottomButtonSize3.y);
			if (Widgets.ButtonText(rect8, "ResetButton".Translate()))
			{
			...
			float x6 = rect7.xMax + 10f;
			float y3 = rect7.y;
			Vector2 otherBottomButtonSize4 = OtherBottomButtonSize;
			float x7 = otherBottomButtonSize4.x;
			Vector2 otherBottomButtonSize5 = OtherBottomButtonSize;
			Rect rect9 = new Rect(x6, y3, x7, otherBottomButtonSize5.y);
			if (Widgets.ButtonText(rect9, "CancelButton".Translate()))
			{
				Close();
				Event.current.Use();
			}
			Vector2 otherBottomButtonSize6 = OtherBottomButtonSize;
			float y4 = otherBottomButtonSize6.y;
			Rect rect10 = new Rect(inRect.width - y4, rect7.y, y4, y4);
			if (Widgets.ButtonImageWithBG(rect10, ShowSellableItemsIcon, new Vector2(32f, 32f)))
			{
			...
			{
				Rect rect11 = new Rect(rect10.x - y4 - 4f, rect7.y, y4, y4);
				if (TradeSession.giftMode)
				{
					if (Widgets.ButtonImageWithBG(rect11, TradeModeIcon, new Vector2(32f, 32f)))
					{
				...
				{
					if (Widgets.ButtonImageWithBG(rect11, GiftModeIcon, new Vector2(32f, 32f)))
					{
