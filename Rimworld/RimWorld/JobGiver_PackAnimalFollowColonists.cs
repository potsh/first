...
namespace RimWorld
{
	public class JobGiver_PackAnimalFollowColonists : ThinkNode_JobGiver
	{
		...
		protected override Job TryGiveJob(Pawn pawn)
		{
			...
			Job job = new Job(JobDefOf.Follow, pawnToFollow);
		...
		public static Pawn GetPawnToFollow(Pawn forPawn)
		{
			...
			for (int i = 0; i < ownedPawns.Count; i++)
			{
				...
				if (pawn != forPawn && CaravanUtility.IsOwner(pawn, forPawn.Faction) && pawn.CurJob != null && pawn.CurJob.def == JobDefOf.PrepareCaravan_GatherItems && forPawn.CanReach(pawn, PathEndMode.Touch, Danger.Deadly))
				{
