...
namespace RimWorld
{
	public class Inspiration : IExposable
	{
		...
		protected virtual void SendBeginLetter()
		{
			...
			{
				string text = def.beginLetter.Formatted(pawn.LabelCap, pawn.Named("PAWN")).AdjustedFor(pawn);
				string label = (def.beginLetterLabel ?? def.LabelCap).CapitalizeFirst() + ": " + pawn.LabelShortCap;
				Find.LetterStack.ReceiveLetter(label, text, def.beginLetterDef, pawn);
			}
		}
		...
		protected virtual void AddEndMessage()
		{
			...
			{
				string text = def.endMessage.Formatted(pawn.LabelCap, pawn.Named("PAWN")).AdjustedFor(pawn);
