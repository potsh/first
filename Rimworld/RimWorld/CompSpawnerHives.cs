...
namespace RimWorld
{
	public class CompSpawnerHives : ThingComp
	{
		...
		private bool CanSpawnChildHive => canSpawnHives && HiveUtility.TotalSpawnedHivesCount(parent.Map) < 30;
		...
		public override void CompTick()
		{
			...
			if ((hive == null || hive.active) && Find.TickManager.TicksGame >= nextHiveSpawnTick)
			{
		...
		public override IEnumerable<Gizmo> CompGetGizmosExtra()
		{
			if (Prefs.DevMode)
			{
				yield return (Gizmo)new Command_Action
				{
					...
					action = delegate
					{
						((_003CCompGetGizmosExtra_003Ec__Iterator0)/*Error near IL_005c: stateMachine*/)._0024this.TrySpawnChildHive(ignoreRoofedRequirement: false, out Hive _);
					}
				};
