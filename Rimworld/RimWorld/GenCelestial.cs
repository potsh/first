...
namespace RimWorld
{
	public static class GenCelestial
	{
		...
		private static Vector3 SunPosition(float latitude, int dayOfYear, float dayPercent)
		{
			...
			Vector3 current = SunPositionUnmodified((float)dayOfYear, dayPercent, new Vector3(1f, 0f, 0f), latitude);
		...
		private static Vector3 SunPositionUnmodified(float dayOfYear, float dayPercent, Vector3 initialSunPos, float latitude = 0f)
		{
			...
			float num2 = 0f - Mathf.Cos(f);
			point.y += num2 * 100f * SunOffsetFractionFromLatitudeCurve.Evaluate(latitude);
		...
		public static void LogSunGlowForYear()
		{
			for (int i = -90; i <= 90; i += 10)
			{
				...
				for (int k = 0; k < 60; k += 5)
				{
					...
					{
						stringBuilder.Append(CelestialSunGlowPercent((float)i, k, (float)l / 24f).ToString("F2").PadRight(6));
					}
					stringBuilder.AppendLine();
				}
		...
		public static void LogSunAngleForYear()
		{
			for (int i = -90; i <= 90; i += 10)
			{
				...
				for (int k = 0; k < 60; k += 5)
				{
					...
					for (int l = 0; l < 24; l += 2)
					{
						float num = Vector3.Angle(SurfaceNormal((float)i), SunPositionUnmodified((float)k, (float)l / 24f, new Vector3(1f, 0f, 0f)));
