namespace Verse.Sound
{
	public class SoundParameterMapping
	{
		...
		{
			string title = ((inParam == null) ? "null" : inParam.Label) + " -> " + ((outParam == null) ? "null" : outParam.Label);
		...
		public void Apply(Sample samp)
		{
			...
			{
				float num = inParam.ValueFor(samp);
				float value = curve.Evaluate(num);
				outParam.SetOn(samp, value);
				if (UnityData.isDebugBuild && curve.HasView)
				{
					curve.View.SetDebugInput(samp, num);
				}
			}
		}
	}
