	...
	public static class MouseoverSounds
	{
		private struct MouseoverRegionCall
		{
			...
			public override string ToString()
			{
				...
				return "(rect=" + rect + ((!mouseIsOver) ? string.Empty : "mouseIsOver") + ")";
			}
		}
		...
		public static void ResolveFrame()
		{
			...
			{
				MouseoverRegionCall mouseoverRegionCall = frameCalls[i];
				if (mouseoverRegionCall.mouseIsOver)
				{
					...
					{
						MouseoverRegionCall mouseoverRegionCall2 = frameCalls[i];
						mouseoverRegionCall2.sound.PlayOneShotOnCamera();
					}
					lastUsedCallInd = i;
