	...
	public static class ForagedFoodPerDayCalculator
	{
		...
		public static Pair<ThingDef, float> ForagedFoodPerDay(List<Pawn> pawns, BiomeDef biome, Faction faction, bool caravanMovingNow, bool caravanNightResting, StringBuilder explanation = null)
		{
			...
			if (explanation != null)
			{
				...
				explanation.AppendLine();
				string text = "TotalNutrition".Translate() + ": " + num2.ToString("0.##") + " / " + "day".Translate();
				if (num2 > 0f)
				{
					string text2 = text;
					text = text2 + "\n= " + biome.LabelCap + ": " + biome.foragedFood.label.CapitalizeFirst() + " x" + num.ToString("0.##") + " / " + "day".Translate();
				}
		...
		public static float GetBaseForagedNutritionPerDay(Pawn p, out bool skip)
		{
			...
			skip = false;
			return (!StatDefOf.ForagedNutritionPerDay.Worker.IsDisabledFor(p)) ? p.GetStatValue(StatDefOf.ForagedNutritionPerDay) : 0f;
		}
