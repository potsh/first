			...
			for (int k = 0; k < rootsWithoutSmallPassages.Count; k++)
			{
				...
				if (!FeatureWorker.visited[num2])
				{
					...
					if (currentGroup.Count >= minSize)
					{
						...
						if (currentGroup.Count <= maxSize)
						{
							float num3 = (float)currentGroup.Count / (float)FeatureWorker.groupSize[num2];
							if (!(num3 > maxPctOfWholeArea) && (def.canTouchWorldEdge || !currentGroup.Any((int x) => worldGrid.IsOnEdge(x))))
							{
