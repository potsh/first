...
namespace RimWorld.Planet
{
	public static class PlanetShapeGenerator
	{
		...
		private static void DoGenerate()
		{
			...
			{
				bool lastPass = i == subdivisionsCount;
				Subdivide(lastPass);
			}
		...
		private static bool ShouldDiscardGeneratedTile(List<int> generatedTileVerts)
		{
			...
			{
				a += verts[generatedTileVerts[i]];
			}
			return !MeshUtility.VisibleForWorldgen(a / (float)generatedTileVerts.Count, radius, viewCenter, viewAngle);
		}
