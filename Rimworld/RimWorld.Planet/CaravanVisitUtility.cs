...
namespace RimWorld.Planet
{
	...
	public static class CaravanVisitUtility
	{
		...
		public static Command TradeCommand(Caravan caravan)
		{
			...
			{
				SettlementBase settlementBase = SettlementVisitedNow(caravan);
				if (settlementBase != null && settlementBase.CanTradeNow)
				{
					Find.WindowStack.Add(new Dialog_Trade(bestNegotiator, settlementBase));
					PawnRelationUtility.Notify_PawnsSeenByPlayer_Letter_Send(settlementBase.Goods.OfType<Pawn>(), "LetterRelatedPawnsTradingWithSettlement".Translate(Faction.OfPlayer.def.pawnsPlural), LetterDefOf.NeutralEvent);
				}
			};
