	...
	public class WorldGenStep_Rivers : WorldGenStep
	{
		...
		private void ExtendRiver(float[] flow, List<int>[] riverPaths, int index, RiverDef incomingRiver)
		{
			if (riverPaths[index] != null)
			{
				...
				{
					foreach (int item in from ni in riverPaths[index]
					where ni != bestOutput
					select ni)
					{
						RiverDef.Branch branch2 = incomingRiver.branches.Where((RiverDef.Branch branch) => (float)branch.minFlow <= flow[item]).MaxByWithFallback((RiverDef.Branch branch) => branch.minFlow);
						if (branch2 != null && Rand.Value < branch2.chance)
						{
							Find.WorldGrid.OverlayRiver(index, item, branch2.child);
							ExtendRiver(flow, riverPaths, item, branch2.child);
						}
					}
				}
			}
