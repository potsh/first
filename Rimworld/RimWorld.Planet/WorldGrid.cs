...
namespace RimWorld.Planet
{
	public class WorldGrid : IExposable
	{
		...
		public Tile this[int tileID]
		{
			get
			{
				return ((uint)tileID >= TilesCount) ? null : tiles[tileID];
			}
		}
		...
		public float GetHeadingFromTo(Vector3 from, Vector3 to)
		{
			...
			float num2 = Vector3.Dot(forward2, right);
			if (num2 < 0f)
			{
				num = 360f - num;
			}
		...
		{
			return tile1 == tile2 || IsNeighbor(tile1, tile2);
		}
		...
		{
			int num = (tileID + 1 >= tileIDToVerts_offsets.Count) ? verts.Count : tileIDToVerts_offsets[tileID + 1];
		...
		public float DistanceFromEquatorNormalized(int tile)
		{
			...
			return Mathf.Abs(tileCenter.y / 100f);
		}
		...
		public int TraversalDistanceBetween(int start, int end, bool passImpassable = true, int maxDist = int.MaxValue)
		{
			...
			if (cachedTraversalDistanceForStart == start && cachedTraversalDistanceForEnd == end && passImpassable && maxDist == 2147483647)
			{
			...
			int maxTilesToProcess = (maxDist != 2147483647) ? TilesNumWithinTraversalDistance(maxDist + 1) : 2147483647;
		...
		{
			return InBounds(tileID) && GetTileNeighborCount(tileID) < GetMaxTileNeighborCountEver(tileID);
		}
		...
		private void CalculateAverageTileSize()
		{
			...
			for (int i = 0; i < tilesCount; i++)
			{
				...
				int num3 = (i + 1 >= tileIDToNeighbors_offsets.Count) ? tileIDToNeighbors_values.Count : tileIDToNeighbors_offsets[i + 1];
		...
		private void TilesToRawData()
		{
			...
			tileElevation = DataSerializeUtility.SerializeUshort(TilesCount, (int i) => (ushort)Mathf.Clamp(Mathf.RoundToInt(((!tiles[i].WaterCovered) ? Mathf.Max(tiles[i].elevation, 1f) : tiles[i].elevation) + 8192f), 0, 65535));
			...
			tileFeature = DataSerializeUtility.SerializeUshort(TilesCount, (int i) => (ushort)((tiles[i].feature != null) ? ((ushort)tiles[i].feature.uniqueID) : 65535));
		...
		private void RawDataToTiles()
		{
			...
			{
				tiles[i].elevation = (float)(data - 8192);
			});
			...
			{
				tiles[i].rainfall = (float)(int)data;
			});
