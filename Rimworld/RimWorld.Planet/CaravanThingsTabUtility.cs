...
namespace RimWorld.Planet
{
	...
	public static class CaravanThingsTabUtility
	{
		...
		{
			Color baseColor = (p != specificTabForPawn) ? Color.white : OpenedSpecificTabButtonColor;
			Color mouseoverColor = (p != specificTabForPawn) ? GenUI.MouseoverColor : OpenedSpecificTabButtonMouseoverColor;
			Rect rect = new Rect(rowRect.width - 24f, (rowRect.height - 24f) / 2f, 24f, 24f);
		...
		{
			float mass = thing.GetStatValue(StatDefOf.Mass) * (float)thing.stackCount;
			DrawMass(mass, rect);
		}
