			...
			while (true)
			{
				...
				if (costNode.cost == calcGrid[costNode.tile].costNodeCost)
				{
					...
					if (calcGrid[tile].status != statusClosedValue)
					{
						...
						{
							Find.WorldDebugDrawer.FlashTile(tile, (float)calcGrid[tile].knownCost / 375000f, calcGrid[tile].knownCost.ToString());
						}
						if (tile == destTile)
						{
						...
						for (int i = tileIDToNeighbors_offsets[tile]; i < num4; i++)
						{
							...
							{
								int num6 = (int)((float)num2 * movementDifficulty[num5] * grid.GetRoadMovementDifficultyMultiplier(tile, num5));
								int num7 = num6 + calcGrid[tile].knownCost;
								ushort status = calcGrid[num5].status;
								if ((status != statusClosedValue && status != statusOpenValue) || calcGrid[num5].knownCost > num7)
								{
									...
									if (status != statusClosedValue && status != statusOpenValue)
									{
										...
										calcGrid[num5].heuristicCost = Mathf.RoundToInt((float)num2 * num8 * (float)num3 * 0.5f);
									}
									int num9 = num7 + calcGrid[num5].heuristicCost;
									calcGrid[num5].parentTile = tile;
									calcGrid[num5].knownCost = num7;
									calcGrid[num5].status = statusOpenValue;
									calcGrid[num5].costNodeCost = num9;
									openList.Push(new CostNode(num5, num9));
								}
							}
						}
						num++;
						...
						if (calcGrid[tile].status != statusClosedValue)
						{
							...
							for (int i = tileIDToNeighbors_offsets[tile]; i < num; i++)
							{
								...
								if (calcGrid[num2].status != statusClosedValue && !impassable(num2))
								{
									...
									int num4 = num3 + calcGrid[tile].knownCost;
									ushort status = calcGrid[num2].status;
									if ((status != statusClosedValue && status != statusOpenValue) || calcGrid[num2].knownCost > num4)
									{
										int num5 = num4;
										calcGrid[num2].parentTile = tile;
										calcGrid[num2].knownCost = num4;
										calcGrid[num2].status = statusOpenValue;
										calcGrid[num2].costNodeCost = num5;
										openList.Push(new CostNode(num2, num5));
									}
								}
							}
							calcGrid[tile].status = statusClosedValue;
