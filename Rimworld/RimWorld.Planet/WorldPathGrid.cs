...
namespace RimWorld.Planet
{
	public class WorldPathGrid
	{
		...
		public void RecalculatePerceivedMovementDifficultyAt(int tile, int? ticksAbs = default(int?))
		{
			if (Find.WorldGrid.InBounds(tile))
			{
				bool flag = PassableFast(tile);
				movementDifficulty[tile] = CalculatedMovementDifficultyAt(tile, perceivedStatic: true, ticksAbs);
				if (flag != PassableFast(tile))
				{
		...
		public static float CalculatedMovementDifficultyAt(int tile, bool perceivedStatic, int? ticksAbs = default(int?), StringBuilder explanation = null)
		{
			...
			return num + GetCurrentWinterMovementDifficultyOffset(tile, (!ticksAbs.HasValue) ? GenTicks.TicksAbs : ticksAbs.Value, explanation);
		}

		public static float GetCurrentWinterMovementDifficultyOffset(int tile, int? ticksAbs = default(int?), StringBuilder explanation = null)
		{
			...
			float yearPct = GenDate.YearPercent(ticksAbs.Value, vector.x);
			SeasonUtility.GetSeason(yearPct, vector.y, out float _, out float _, out float _, out float winter, out float _, out float permanentWinter);
