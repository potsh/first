...
namespace RimWorld.Planet
{
	public class Caravan_PathFollower : IExposable
	{
		...
		public bool Moving => moving && caravan.Spawned;

		public bool MovingNow => Moving && !Paused && !caravan.CantMove;

		public CaravanArrivalAction ArrivalAction => (!Moving) ? null : arrivalAction;
		...
		public bool StartPath(int destTile, CaravanArrivalAction arrivalAction, bool repathImmediately = false, bool resetPauseStatus = true)
		{
			...
			if (arrivalAction != null && !(bool)arrivalAction.StillValid(caravan, destTile))
			{
				return false;
			}
		...
		{
			if (moving && arrivalAction != null && !(bool)arrivalAction.StillValid(caravan, Destination))
			{
				string failMessage = arrivalAction.StillValid(caravan, Destination).FailMessage;
				Messages.Message("MessageCaravanArrivalActionNoLongerValid".Translate(caravan.Name).CapitalizeFirst() + ((failMessage == null) ? string.Empty : (" " + failMessage)), caravan, MessageTypeDefOf.NegativeEvent);
				StopDead();
			}
		...
		private void PatherArrived()
		{
			...
			{
				Messages.Message("MessageCaravanArrivedAtDestination".Translate(caravan.Label).CapitalizeFirst(), caravan, MessageTypeDefOf.TaskCompletion);
			}
		}
		...
		public static int CostToMove(Caravan caravan, int start, int end, int? ticksAbs = default(int?))
		{
			return CostToMove(caravan.TicksPerMove, start, end, ticksAbs);
		}

		public static int CostToMove(int caravanTicksPerMove, int start, int end, int? ticksAbs = default(int?), bool perceivedStatic = false, StringBuilder explanation = null, string caravanTicksPerMoveExplanation = null)
		{
			if (start == end)
			{
				return 0;
		...
		{
			int num = (!moving || nextTile < 0 || !IsNextTilePassable()) ? caravan.Tile : nextTile;
