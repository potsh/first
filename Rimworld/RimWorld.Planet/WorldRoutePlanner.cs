...
namespace RimWorld.Planet
{
	...
	public class WorldRoutePlanner
	{
		...
		public bool FormingCaravan => Active && currentFormCaravanDialog != null;
		...
		public void Stop()
		{
			...
			{
				worldObjects.Remove(waypoints[i]);
			}
			waypoints.Clear();
		...
		private bool DoChooseRouteButton()
		{
			...
			float num2 = (float)UI.screenHeight;
			Vector2 bottomWindowSize = BottomWindowSize;
			float num3 = num2 - bottomWindowSize.y - 45f - 10f;
			Vector2 bottomButtonSize2 = BottomButtonSize;
			float y = num3 - bottomButtonSize2.y;
			Vector2 bottomButtonSize3 = BottomButtonSize;
			float x2 = bottomButtonSize3.x;
			Vector2 bottomButtonSize4 = BottomButtonSize;
			Rect rect = new Rect(x, y, x2, bottomButtonSize4.y);
			if (Widgets.ButtonText(rect, "ChooseRouteButton".Translate()))
			{
				Find.WindowStack.Add(currentFormCaravanDialog);
		...
		{
			float num = (float)ButtonTex.width;
			float num2 = (float)ButtonTex.height;
		...
		private void TryAddWaypoint(int tile, bool playSound = true)
		{
			...
			else if (waypoints.Any() && !Find.WorldReachability.CanReach(waypoints[waypoints.Count - 1].Tile, tile))
			{
				Messages.Message("MessageCantAddWaypointBecauseUnreachable".Translate(), MessageTypeDefOf.RejectInput, historical: false);
			}
			else if (waypoints.Count >= 25)
			{
