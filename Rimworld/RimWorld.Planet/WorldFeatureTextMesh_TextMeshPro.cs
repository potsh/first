...
namespace RimWorld.Planet
{
	...
	public class WorldFeatureTextMesh_TextMeshPro : WorldFeatureTextMesh
	{
		...
		{
			GameObject gameObject = Object.Instantiate(WorldTextPrefab);
			...
			if (characterCount != 0)
			{
				...
				for (int i = 0; i < characterCount; i++)
				{
					...
					if (tMP_CharacterInfo.isVisible)
					{
						...
						Vector3 a = textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex] + textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 1] + textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 2] + textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 3];
						a /= 4f;
						float num3 = a.x / (num / 2f);
						bool flag = num3 >= 0f;
						num3 = Mathf.Abs(num3);
						float num4 = num2 / 2f * num3;
						float num5 = (180f - num4) / 2f;
						float num6 = 200f * Mathf.Tan(num4 / 2f * 0.0174532924f);
						Vector3 vector = new Vector3(Mathf.Sin(num5 * 0.0174532924f) * num6 * ((!flag) ? (-1f) : 1f), a.y, Mathf.Cos(num5 * 0.0174532924f) * num6);
						Vector3 b = vector - a;
						Vector3 a2 = textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex] + b;
						Vector3 a3 = textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 1] + b;
						Vector3 a4 = textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 2] + b;
						Vector3 a5 = textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 3] + b;
						...
						a5 = worldToLocalMatrix.MultiplyPoint(localToWorldMatrix.MultiplyPoint(a5).normalized * (100f + WorldAltitudeOffsets.WorldText));
						textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex] = a2;
						textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 1] = a3;
						textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 2] = a4;
						textMesh.textInfo.meshInfo[materialReferenceIndex].vertices[vertexIndex + 3] = a5;
					}
				}
