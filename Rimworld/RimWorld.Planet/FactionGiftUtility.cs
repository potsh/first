...
namespace RimWorld.Planet
{
	...
	public static class FactionGiftUtility
	{
		...
		public static void GiveGift(List<Tradeable> tradeables, Faction giveTo, GlobalTargetInfo lookTarget)
		{
			...
			GlobalTargetInfo? lookTarget2 = lookTarget;
			if (!giveTo.TryAffectGoodwillWith(ofPlayer, goodwillChange2, canSendMessage: true, canSendHostilityLetter: true, reason, lookTarget2))
			{
		...
		public static void GiveGift(List<ActiveDropPodInfo> pods, SettlementBase giveTo)
		{
			...
			int goodwillChange2 = goodwillChange;
			string reason = "GoodwillChangedReason_ReceivedGift".Translate();
			GlobalTargetInfo? lookTarget = giveTo;
			if (!faction.TryAffectGoodwillWith(ofPlayer, goodwillChange2, canSendMessage: true, canSendHostilityLetter: true, reason, lookTarget))
			{
		...
		{
			Messages.Message("MessageGiftGivenButNotAppreciated".Translate(giveTo.Name).CapitalizeFirst(), lookTarget, MessageTypeDefOf.NegativeEvent);
		}
	}
}
