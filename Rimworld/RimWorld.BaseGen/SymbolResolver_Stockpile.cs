		...
		public override void Resolve(ResolveParams rp)
		{
			...
			else
			{
				...
				else
				{
					...
					value.validator = delegate(ThingDef x)
					{
						if (rp.faction != null && (int)x.techLevel < (int)rp.faction.def.techLevel && x.IsWeapon && x.GetStatValueAbstract(StatDefOf.MarketValue, GenStuff.DefaultStuffFor(x)) < 100f)
						{
							return false;
						}
						return true;
					...
					float num4 = (!stockpileMarketValue.HasValue) ? Mathf.Min((float)cells.Count * 130f, 1800f) : stockpileMarketValue.Value;
