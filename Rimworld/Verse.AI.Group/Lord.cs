...
namespace Verse.AI.Group
{
	...
	public class Lord : IExposable, ILoadReferenceable
	{
		...
		public void GotoToil(LordToil newLordToil)
		{
			...
			{
				Log.Error("curLordToil lord is " + ((curLordToil.lord != null) ? curLordToil.lord.ToString() : "null (forgot to add toil to graph?)"));
		...
		private Vector3 DebugCenter()
		{
			...
			if ((from p in ownedPawns
			where p.Spawned
			select p).Any())
			{
				result.x = (from p in ownedPawns
				where p.Spawned
