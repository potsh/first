	...
	public class Noise2D : IDisposable
	{
		...
		public void GeneratePlanar(double left, double right, double top, double bottom, bool isSeamless)
		{
			...
			for (int i = 0; i < m_ucWidth; i++)
			{
				...
				for (int j = 0; j < m_ucHeight; j++)
				{
					if (isSeamless)
					{
						num6 = (float)GeneratePlanar(num5, num7);
					}
					else
					{
						double a = GeneratePlanar(num5, num7);
						double b = GeneratePlanar(num5 + num, num7);
						double a2 = GeneratePlanar(num5, num7 + num2);
						double b2 = GeneratePlanar(num5 + num, num7 + num2);
					...
					{
						m_data[i - m_ucBorder, j - m_ucBorder] = num6;
					}
					num7 += num4;
				}
				num5 += num3;
		...
		public void GenerateCylindrical(double angleMin, double angleMax, double heightMin, double heightMax)
		{
			...
			for (int i = 0; i < m_ucWidth; i++)
			{
				...
				for (int j = 0; j < m_ucHeight; j++)
				{
					float[,] ucData = m_ucData;
					int num7 = i;
					int num8 = j;
					float num9 = (float)GenerateCylindrical(num5, num6);
					ucData[num7, num8] = num9;
					if (i >= m_ucBorder && j >= m_ucBorder && i < m_width + m_ucBorder && j < m_height + m_ucBorder)
					{
						float[,] data = m_data;
						int num10 = i - m_ucBorder;
						int num11 = j - m_ucBorder;
						float num12 = (float)GenerateCylindrical(num5, num6);
		...
		private double GenerateSpherical(double lat, double lon)
		{
			...
			return m_generator.GetValue(num * Math.Cos(0.017453292519943295 * lon), Math.Sin(0.017453292519943295 * lat), num * Math.Sin(0.017453292519943295 * lon));
		}
		...
		public void GenerateSpherical(double south, double north, double west, double east)
		{
			...
			for (int i = 0; i < m_ucWidth; i++)
			{
				...
				for (int j = 0; j < m_ucHeight; j++)
				{
					...
					int num7 = i;
					int num8 = j;
					float num9 = (float)GenerateSpherical(num6, num5);
					ucData[num7, num8] = num9;
					if (i >= m_ucBorder && j >= m_ucBorder && i < m_width + m_ucBorder && j < m_height + m_ucBorder)
					{
						float[,] data = m_data;
						int num10 = i - m_ucBorder;
						int num11 = j - m_ucBorder;
						float num12 = (float)GenerateSpherical(num6, num5);
						data[num10, num11] = num12;
					}
					num6 += num4;
				}
