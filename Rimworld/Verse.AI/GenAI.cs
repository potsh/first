	...
	public static class GenAI
	{
		...
		public static IntVec3 RandomRaidDest(IntVec3 raidSpawnLoc, Map map)
		{
			...
			for (int i = 0; i < allBedDefBestToWorst.Count; i++)
			{
				...
				{
					if (((Building_Bed)item).owners.Any() && map.reachability.CanReach(raidSpawnLoc, item, PathEndMode.OnCell, TraverseMode.PassAllDestroyableThings, Danger.Deadly))
					{
						list.Add(item);
					}
				}
			...
			select x).TryRandomElement(out Pawn result2))
			{
