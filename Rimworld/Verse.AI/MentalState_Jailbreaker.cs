...
namespace Verse.AI
{
	public class MentalState_Jailbreaker : MentalState
	{
		...
		public void Notify_InducedPrisonerToEscape()
		{
			...
			{
				pawn.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.Wander_OwnRoom, null, forceWake: false, causedByMood: false, null, transitionSilently: true);
			}
			else if (MentalStateDefOf.Wander_Sad.Worker.StateCanOccur(pawn))
			{
				pawn.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.Wander_Sad, null, forceWake: false, causedByMood: false, null, transitionSilently: true);
			}
			else
			{
				RecoverFromState();
