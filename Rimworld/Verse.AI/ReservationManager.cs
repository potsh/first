...
namespace Verse.AI
{
	...
	public sealed class ReservationManager : IExposable
	{
		...
		public bool ReservedBy<TDriver>(LocalTargetInfo target, Pawn claimant, LocalTargetInfo? targetAIsNot = default(LocalTargetInfo?), LocalTargetInfo? targetBIsNot = default(LocalTargetInfo?), LocalTargetInfo? targetCIsNot = default(LocalTargetInfo?))
		{
			if (!target.IsValid)
			{
				return false;
