...
namespace Verse.AI
{
	public static class ReservationUtility
	{
		...
		public static bool CanReserveAndReach(this Pawn p, LocalTargetInfo target, PathEndMode peMode, Danger maxDanger, int maxPawns = 1, int stackCount = -1, ReservationLayerDef layer = null, bool ignoreOtherReservations = false)
		{
			...
			return p.CanReach(target, peMode, maxDanger) && p.Map.reservationManager.CanReserve(p, target, maxPawns, stackCount, layer, ignoreOtherReservations);
		}
		...
		public static bool HasReserved<TDriver>(this Pawn p, LocalTargetInfo target, LocalTargetInfo? targetAIsNot = default(LocalTargetInfo?), LocalTargetInfo? targetBIsNot = default(LocalTargetInfo?), LocalTargetInfo? targetCIsNot = default(LocalTargetInfo?))
		{
			if (!p.Spawned)
			{
				return false;
		...
		{
			return target.IsValid && !p.HasReserved(target) && p.CanReserve(target);
		}
	}
}
