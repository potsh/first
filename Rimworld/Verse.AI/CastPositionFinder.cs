...
namespace Verse.AI
{
	public static class CastPositionFinder
	{
		...
		private static void EvaluateCell(IntVec3 c)
		{
			...
			else if ((double)maxRangeFromLocusSquared > 0.01 && (float)(c - req.locus).LengthHorizontalSquared > maxRangeFromLocusSquared)
			{
				if (DebugViewSettings.drawCastPositionSearch)
				{
			...
			else
			{
				...
				{
					rangeFromCasterToCellSquared = (float)(c - req.caster.Position).LengthHorizontalSquared;
				...
				if (c.Walkable(req.caster.Map))
				{
					...
					else if (!req.caster.Map.reachability.CanReach(req.caster.Position, c, PathEndMode.OnCell, TraverseParms.For(req.caster, Danger.Some)))
					{
						if (DebugViewSettings.drawCastPositionSearch)
						{
		...
		private static float CastPositionPreference(IntVec3 c)
		{
			...
			float num3 = 1f;
			rangeFromTargetToCellSquared = (float)(c - req.target.Position).LengthHorizontalSquared;
