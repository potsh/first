...
namespace Verse.AI
{
	public abstract class JobDriver : IExposable, IJobEndable
	{
		...
		protected Toil CurToil
		{
			get
			{
				if (curToilIndex < 0 || pawn.CurJob != job)
				{
					return null;
				}
		...
		protected bool HaveCurToil => curToilIndex >= 0 && curToilIndex < toils.Count && pawn.CurJob == job;
		...
		public SkillDef ActiveSkill => (!HaveCurToil || CurToil.activeSkill == null) ? null : CurToil.activeSkill();
		...
		protected string ReportStringProcessed(string str)
		{
			...
			invalid = ((!job.targetA.IsValid) ? job.targetQueueA.FirstValid() : job.targetA);
			str = ((!invalid.IsValid) ? str.Replace("TargetA", "UnknownLower".Translate()) : ((!invalid.HasThing) ? str.Replace("TargetA", "AreaLower".Translate()) : str.Replace("TargetA", invalid.Thing.LabelShort)));
			LocalTargetInfo invalid2 = LocalTargetInfo.Invalid;
			invalid2 = ((!job.targetB.IsValid) ? job.targetQueueB.FirstValid() : job.targetB);
			str = ((!invalid2.IsValid) ? str.Replace("TargetB", "UnknownLower".Translate()) : ((!invalid2.HasThing) ? str.Replace("TargetB", "AreaLower".Translate()) : str.Replace("TargetB", invalid2.Thing.LabelShort)));
			LocalTargetInfo targetC = job.targetC;
			str = ((!targetC.IsValid) ? str.Replace("TargetC", "UnknownLower".Translate()) : ((!targetC.HasThing) ? str.Replace("TargetC", "AreaLower".Translate()) : str.Replace("TargetC", targetC.Thing.LabelShort)));
			return str;
		}
		...
		{
			if (pawn.Spawned && (!pawn.stances.FullBodyBusy || CanStartNextToilInBusyStance) && pawn.CurJob == job)
			{
				if (HaveCurToil)
				{
				...
				else
				{
					...
					if (!CheckCurrentToilEndOrFail())
					{
						...
						if (CurToil == curToil)
						{
							...
							if (CurToil == curToil && !ended && curToilCompleteMode == ToilCompleteMode.Instant)
							{
								ReadyForNextToil();
							}
						}
		...
		{
			if (!pawn.Destroyed && pawn.CurJob == job)
			{
