	...
	public static class TantrumMentalStateUtility
	{
			...
			if (pawn.CanReach(near, PathEndMode.OnCell, Danger.Deadly))
			{
				...
				if (region != null)
				{
					...
					RegionTraverser.BreadthFirstTraverse(region, (Region from, Region to) => to.Allows(traverseParams, isDestination: false), delegate(Region r)
					{
						...
						{
							if (list[i].Position.InHorDistOf(near, (float)maxDistance) && CanSmash(pawn, list[i], skipReachabilityCheck: true, customValidator, extraMinBuildingOrItemMarketValue))
							{
						...
						{
							if (list2[j].Position.InHorDistOf(near, (float)maxDistance) && CanSmash(pawn, list2[j], skipReachabilityCheck: true, customValidator, extraMinBuildingOrItemMarketValue))
							{
						...
						{
							if (list3[k].Position.InHorDistOf(near, (float)maxDistance) && CanSmash(pawn, list3[k], skipReachabilityCheck: true, customValidator, extraMinBuildingOrItemMarketValue))
							{
		...
		public static void GetSmashableThingsIn(Room room, Pawn pawn, List<Thing> outCandidates, Predicate<Thing> customValidator = null, int extraMinBuildingOrItemMarketValue = 0)
		{
			...
			for (int i = 0; i < containedAndAdjacentThings.Count; i++)
			{
				...
				if (CanSmash(pawn, thing, skipReachabilityCheck: false, customValidator, extraMinBuildingOrItemMarketValue))
				{
