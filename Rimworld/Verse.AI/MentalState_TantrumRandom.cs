...
namespace Verse.AI
{
	public abstract class MentalState_TantrumRandom : MentalState_Tantrum
	{
		...
		private void ChooseNextTarget()
		{
			...
			{
				Thing thing = (target == null || Find.TickManager.TicksGame - targetFoundTicks <= 1250 || !candidates.Any((Thing x) => x != target)) ? candidates.RandomElementByWeight((Thing x) => GetCandidateWeight(x)) : (from x in candidates
				where x != target
				select x).RandomElementByWeight((Thing x) => GetCandidateWeight(x));
				if (thing != target)
				{
					target = thing;
					hitTargetAtLeastOnce = false;
