	...
	public class RegionCostCalculator
	{
				...
				if (regionLinkQueueEntry.Cost == num)
				{
					...
					if (otherRegion != null && otherRegion.valid)
					{
						...
						for (int i = 0; i < otherRegion.links.Count; i++)
						{
							...
							{
								int val = (otherRegion.door == null) ? RegionLinkDistance(regionLinkQueueEntry.Link, regionLink, minPathCost) : num2;
								val = Math.Max(val, 1);
								int num3 = num + val;
		...
		public int GetRegionBestDistances(Region region, out RegionLink bestLink, out RegionLink secondBestLink, out int secondBestCost)
		{
			...
			for (int i = 0; i < region.links.Count; i++)
			{
				...
				int value;
				if (regionLink != bestLink && regionLink.GetOtherRegion(region).type.Passable() && distances.TryGetValue(regionLink, out value) && value < secondBestCost)
				{
		...
		{
			return (float)(GetCellCostFast(b) + ((!AreCellsDiagonal(a, b)) ? moveTicksCardinal : moveTicksDiagonal));
		}
		...
		private List<int> PathableNeighborIndices(int index)
		{
			...
			bool flag6 = !flag2 || PathFinder.BlocksDiagonalMovement(index + 1, map);
			...
			if (flag4 && !PathFinder.BlocksDiagonalMovement(index + x, map))
			{
				if (!flag6 && pathGrid.WalkableFast(index + x + 1))
				{
					tmpPathableNeighborIndices.Add(index + x + 1);
