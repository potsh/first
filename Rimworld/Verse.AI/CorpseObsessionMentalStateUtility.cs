	...
	public static class CorpseObsessionMentalStateUtility
	{
		...
		public static bool IsCorpseValid(Corpse corpse, Pawn pawn, bool ignoreReachability = false)
		{
			...
			if (corpse.Spawned)
			{
				...
				return ignoreReachability || pawn.CanReach(corpse, PathEndMode.Touch, Danger.Deadly);
			}
			...
			if (building_Grave != null && building_Grave.Spawned)
			{
				...
				return ignoreReachability || pawn.CanReach(building_Grave, PathEndMode.InteractionCell, Danger.Deadly);
			}
			return false;
		}
	}
