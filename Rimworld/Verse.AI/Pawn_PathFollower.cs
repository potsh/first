...
namespace Verse.AI
{
	public class Pawn_PathFollower : IExposable
	{
		...
		public void StartPath(LocalTargetInfo dest, PathEndMode peMode)
		{
			...
			else if ((PawnCanOccupy(pawn.Position) || TryRecoverFromUnwalkablePosition()) && (!moving || curPath == null || !(destination == dest) || this.peMode != peMode))
			{
				...
				else
				{
					this.peMode = peMode;
					destination = dest;
					if (!IsNextCellWalkable() || NextCellDoorToManuallyOpen() != null || nextCellCostLeft == nextCellCostTotal)
					{
		...
		public Building_Door NextCellDoorToManuallyOpen()
		{
			...
			if (building_Door != null && building_Door.SlowsPawns && !building_Door.Open && building_Door.PawnCanOpen(pawn))
			{
		...
		private void TryEnterNextPathCell()
		{
			...
			Building_Door building_Door2 = NextCellDoorToManuallyOpen();
