...
namespace Verse
{
	public class EditWindow_CurveEditor : EditWindow
	{
		...
		private void DoCurveEditor(Rect screenRect)
		{
			...
			string s = Widgets.TextField(rect2, curve.View.rect.x.ToString());
			...
			rect2.y = screenRect.y + screenRect.height / 2f - 12f;
			s = Widgets.TextField(rect2, curve.View.rect.xMax.ToString());
			...
			s = Widgets.TextField(rect2, curve.View.rect.y.ToString());
			if (float.TryParse(s, out result))
			{
			...
			s = Widgets.TextField(rect2, curve.View.rect.yMax.ToString());
			if (float.TryParse(s, out result))
			{
			...
			if (Mouse.IsOver(screenRect))
			{
				if (Event.current.type == EventType.ScrollWheel)
				{
					...
					float num = -1f * delta.y * 0.025f;
					Vector2 center = curve.View.rect.center;
					float num2 = center.x - curve.View.rect.x;
					Vector2 center2 = curve.View.rect.center;
					float num3 = center2.y - curve.View.rect.y;
			...
			if (DraggingView)
			{
				if (Event.current.type == EventType.MouseDrag)
				{
					...
					curve.View.rect.x -= delta2.x * curve.View.rect.width * 0.002f;
					curve.View.rect.y += delta2.y * curve.View.rect.height * 0.002f;
					Event.current.Use();
				}
