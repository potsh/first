...
namespace Verse
{
	public static class TranslationHandleUtility
	{
		...
		public static int GetElementIndexByHandle(object list, string handle, int handleIndex)
		{
			...
			for (int k = 0; k < num; k++)
			{
				...
				if (value3 != null)
				{
					...
					foreach (FieldInfo fieldInfo3 in array2)
					{
		...
		public static bool HandlesMatch(object item, string handle)
		{
			...
			if (type != null)
			{
				return NormalizedHandle(type.Name) == handle || NormalizedHandle(type.FullName) == handle || NormalizedHandle(type.ToString()) == handle;
			}
			string text;
			try
			{
		...
		private static string NormalizedHandle(string handle)
		{
			...
			if (handle.IndexOf('-') >= 0)
			{
				handle = handle.Replace(45.ToString(), string.Empty);
			}
			if (handle.IndexOf("{") >= 0)
			{
