...
namespace Verse
{
	public static class GenPlace
	{
		...
		public static bool TryPlaceThing(Thing thing, IntVec3 center, Map map, ThingPlaceMode mode, Action<Thing, int> placedAction = null, Predicate<IntVec3> nearPlaceValidator = null)
		{
			...
			return TryPlaceThing(thing, center, map, mode, out lastResultingThing, placedAction, nearPlaceValidator);
		}
		...
		public static bool TryPlaceThing(Thing thing, IntVec3 center, Map map, ThingPlaceMode mode, out Thing lastResultingThing, Action<Thing, int> placedAction = null, Predicate<IntVec3> nearPlaceValidator = null)
		{
			if (map != null)
			{
				...
				case ThingPlaceMode.Direct:
					return TryPlaceDirect(thing, center, map, out lastResultingThing, placedAction);
				case ThingPlaceMode.Near:
				{
					...
					do
					{
						...
						if (!TryFindPlaceSpotNear(center, map, thing, allowStacking: true, out IntVec3 bestSpot, nearPlaceValidator))
						{
							return false;
						}
						if (TryPlaceDirect(thing, bestSpot, map, out lastResultingThing, placedAction))
						{
							return true;
						}
					}
		...
		private static bool TryFindPlaceSpotNear(IntVec3 center, Map map, Thing thing, bool allowStacking, out IntVec3 bestSpot, Predicate<IntVec3> extraValidator = null)
		{
			...
			for (int i = 0; i < 9; i++)
			{
				...
				PlaceSpotQuality placeSpotQuality2 = PlaceSpotQualityAt(intVec, map, thing, center, allowStacking, extraValidator);
