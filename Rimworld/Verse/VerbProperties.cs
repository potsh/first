...
namespace Verse
{
	public class VerbProperties
	{
		...
		public bool CausesTimeSlowdown => ai_IsWeapon && forceNormalTimeSpeed;
		...
		public bool CausesExplosion
		{
			get
			{
				...
				return typeof(Projectile_Explosive).IsAssignableFrom(defaultProjectile.thingClass) || typeof(Projectile_DoomsdayRocket).IsAssignableFrom(defaultProjectile.thingClass);
			}
		}
		...
		public float AdjustedArmorPenetration(Tool tool, Pawn attacker, Thing equipment, HediffComp_VerbGiver hediffCompSource)
		{
			...
			if (num < 0f)
			{
				float num2 = AdjustedMeleeDamageAmount(tool, attacker, equipment, hediffCompSource);
				num = num2 * 0.015f;
			}
			return num;
		}
		...
		private float AdjustedExpectedDamageForVerbUsableInMelee(Tool tool, Pawn attacker, Thing equipment, HediffComp_VerbGiver hediffCompSource)
		{
			...
			{
				return (float)defaultProjectile.projectile.GetDamageAmount(equipment);
			}
			return 0f;
		}
		...
		public void DrawRadiusRing(IntVec3 center)
		{
			if (Find.CurrentMap != null && !IsMeleeAttack)
			{
				...
				if (num2 < (float)(x + size2.z) && range < GenRadial.MaxRadialPatternRadius)
				{
		...
		{
			if (parent.race != null && linkedBodyPartsGroup != null && !parent.race.body.AllParts.Any((BodyPartRecord part) => part.groups.Contains(((_003CConfigErrors_003Ec__Iterator0)/*Error near IL_005a: stateMachine*/)._0024this.linkedBodyPartsGroup)))
			{
