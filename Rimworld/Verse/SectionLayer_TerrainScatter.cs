	...
	public class SectionLayer_TerrainScatter : SectionLayer
	{
		private class Scatterable
		{
			...
			public bool IsOnValidTerrain
			{
				get
				{
					...
					return def.scatterType == terrainDef.scatterType && !c.Filled(map);
				}
			}
		...
		public override void Regenerate()
		{
			...
			while (scats.Count < num && num2 < 200)
			{
				...
				select def).TryRandomElement(out ScatterableDef result))
				{
					Scatterable scatterable = new Scatterable(loc: new Vector3((float)randomCell.x + Rand.Value, (float)randomCell.y, (float)randomCell.z + Rand.Value), def: result, map: base.Map);
