...
namespace Verse
{
	...
	public class EditWindow_Log : EditWindow
	{
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (canAutoOpen)
			{
				if (widgetRow.ButtonText("Auto-open is ON", string.Empty))
				{
					canAutoOpen = false;
				}
			}
			else if (widgetRow.ButtonText("Auto-open is OFF", string.Empty))
			{
				canAutoOpen = true;
			}
		...
		private void DoMessagesListing(Rect listingRect)
		{
			...
			foreach (LogMessage message in Log.Messages)
			{
				...
				{
					GUI.DrawTexture(rect2, SelectedMessageTex);
				}
				else if (flag)
				{
					GUI.DrawTexture(rect2, AltMessageTex);
				}
		...
		private void DoMessageDetails(Rect detailsRect, Rect outRect)
		{
			if (selectedMessage != null)
			{
				...
				if (borderDragging)
				{
					...
					detailsPaneHeight = num - mousePosition.y;
				}
