...
namespace Verse
{
	public sealed class GlowGrid
	{
		...
		public float GameGlowAt(IntVec3 c, bool ignoreCavePlants = false)
		{
			...
			Color32[] array = (!ignoreCavePlants) ? glowGrid : glowGridNoCavePlants;
			Color32 color = array[map.cellIndices.CellToIndex(c)];
			if (color.a == 1)
			{
				return 1f;
			}
		...
		{
			float glow = GameGlowAt(c);
			return PsychGlowAtGlow(glow);
		}
