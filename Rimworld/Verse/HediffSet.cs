	...
	public class HediffSet : IExposable
	{
		...
		public IEnumerable<Hediff_Injury> GetInjuriesTendable()
		{
			...
			while (true)
			{
				...
				inj = (hediffs[i] as Hediff_Injury);
		...
		public BodyPartRecord GetRandomNotMissingPart(DamageDef damDef, BodyPartHeight height = BodyPartHeight.Undefined, BodyPartDepth depth = BodyPartDepth.Undefined, BodyPartRecord partParent = null)
		{
			...
			BodyPartHeight height2 = height;
			BodyPartDepth depth2 = depth;
			if (GetNotMissingParts(height2, depth2, null, partParent).Any())
			{
				height2 = height;
				depth2 = depth;
				enumerable = GetNotMissingParts(height2, depth2, null, partParent);
			}
			else
			{
				depth2 = depth;
				if (!GetNotMissingParts(BodyPartHeight.Undefined, depth2, null, partParent).Any())
				{
					return null;
				}
				height2 = BodyPartHeight.Undefined;
				depth2 = depth;
				enumerable = GetNotMissingParts(height2, depth2, null, partParent);
			}
