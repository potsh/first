...
namespace Verse
{
	...
	public static class MapEdgeClipDrawer
	{
		...
		public static void DrawClippers(Map map)
		{
			...
			matrix = default(Matrix4x4);
			matrix.SetTRS(new Vector3((float)(size.x / 2), ClipAltitude, (float)size.z + 250f), Quaternion.identity, s);
			Graphics.DrawMesh(MeshPool.plane10, matrix, ClipMat, 0);
			matrix = default(Matrix4x4);
			matrix.SetTRS(new Vector3((float)(size.x / 2), ClipAltitude, -250f), Quaternion.identity, s);
			Graphics.DrawMesh(MeshPool.plane10, matrix, ClipMat, 0);
		}
	}
}
