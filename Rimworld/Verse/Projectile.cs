...
namespace Verse
{
	public abstract class Projectile : ThingWithComps
	{
		...
		protected int StartingTicksToImpact
		{
			get
			{
				int num = Mathf.RoundToInt((origin - destination).magnitude / (def.projectile.speed / 100f));
				if (num < 1)
				{
					num = 1;
				}
		...
		public override void Draw()
		{
			...
			Graphics.DrawMesh(mesh, DrawPos, ExactRotation, def.DrawMatSingle, 0);
			Comps_PostDraw();
		}
