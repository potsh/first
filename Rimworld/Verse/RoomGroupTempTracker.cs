...
namespace Verse
{
	public sealed class RoomGroupTempTracker
	{
		...
		public float Temperature
		{
			...
			set
			{
				temperatureInt = Mathf.Clamp(value, -273.15f, 2000f);
			}
		}
		...
		private void RegenerateEqualizationData()
		{
			...
			if (this.roomGroup.RoomCount != 0)
			{
				...
				if (!this.roomGroup.UsesOutdoorTemperature)
				{
					...
					thickRoofCoverage /= (float)num;
					noRoofCoverage /= (float)num;
		...
		private float WallEqualizationTempChangePerInterval()
		{
			...
			float num4 = num3 * (float)equalizeCells.Count;
			return num4 * 120f * 0.00017f / (float)roomGroup.CellCount;
		}
		...
		private float ThinRoofEqualizationTempChangePerInterval()
		{
			...
			float num2 = num * ThinRoofCoverage * 5E-05f;
			return num2 * 120f;
		}
		...
		private float NoRoofEqualizationTempChangePerInterval()
		{
			...
			float num = TempDiffFromOutdoorsAdjusted();
			float num2 = num * noRoofCoverage * 0.0007f;
			return num2 * 120f;
		}
		...
		private float DeepEqualizationTempChangePerInterval()
		{
			...
			float num2 = num * thickRoofCoverage * 5E-05f;
			return num2 * 120f;
		}
