					...
					if (DebugViewSettings.writeAttackTargets)
					{
						foreach (Pawn item in Find.CurrentMap.thingGrid.ThingsAt(UI.MouseCell()).OfType<Pawn>())
						{
							...
							for (int i = 0; i < potentialTargetsFor.Count; i++)
							{
								...
								stringBuilder.AppendLine(thing.LabelShort + ", " + thing.Faction + ((!potentialTargetsFor[i].ThreatDisabled(null)) ? string.Empty : " (threat disabled)"));
							}
						}
					}
					...
					if (DebugViewSettings.writeLinkFlags)
					{
						...
						try
						{
							while (enumerator11.MoveNext())
							{
								...
								if ((Find.CurrentMap.linkGrid.LinkFlagsAt(intVec) & (LinkFlags)current11) != 0)
								{
