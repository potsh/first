		...
		private static void PassPawnsToWorld(Map map)
		{
			...
			if (list.Any() || list2.Any())
			{
				...
				if (list.Any())
				{
					...
					{
						stringBuilder.AppendLine("  - " + list[j].LabelCap + " (" + "capturedBy".Translate(map.ParentFaction.Name) + ")");
					}
				}
				if (list2.Any())
				{
				...
				if (map.IsPlayerHome)
				{
					...
					str = "LetterPawnsLostBecauseMapClosed_Home".Translate();
				}
				else
				{
					label = "LetterLabelPawnsLostBecauseMapClosed_Caravan".Translate();
					str = "LetterPawnsLostBecauseMapClosed_Caravan".Translate();
				}
				str = str + ":\n\n" + stringBuilder.ToString().TrimEndNewlines();
				Find.LetterStack.ReceiveLetter(label, str, LetterDefOf.NegativeEvent, new GlobalTargetInfo(map.Tile));
			}
		}
