...
namespace Verse
{
	...
	public static class DebugOutputsEcology
	{
		...
		{
			DebugTables.MakeTablesDialog(from d in DefDatabase<BiomeDef>.AllDefs
			orderby d.plantDensity descending
			select d, new TableDataGetter<BiomeDef>("defName", (BiomeDef d) => d.defName), new TableDataGetter<BiomeDef>("animalDensity", (BiomeDef d) => d.animalDensity.ToString("F2")), new TableDataGetter<BiomeDef>("plantDensity", (BiomeDef d) => d.plantDensity.ToString("F2")), new TableDataGetter<BiomeDef>("diseaseMtbDays", (BiomeDef d) => d.diseaseMtbDays.ToString("F0")), new TableDataGetter<BiomeDef>("movementDifficulty", (BiomeDef d) => (!d.impassable) ? d.movementDifficulty.ToString("F1") : "-"), new TableDataGetter<BiomeDef>("forageability", (BiomeDef d) => d.forageability.ToStringPercent()), new TableDataGetter<BiomeDef>("forageFood", (BiomeDef d) => (d.foragedFood == null) ? string.Empty : d.foragedFood.label), new TableDataGetter<BiomeDef>("forageable plants", (BiomeDef d) => d.AllWildPlants.Where((ThingDef pd) => pd.plant.harvestedThingDef != null && pd.plant.harvestedThingDef.IsNutritionGivingIngestible).Select((ThingDef pd) => pd.defName).ToCommaList()), new TableDataGetter<BiomeDef>("wildPlantRegrowDays", (BiomeDef d) => d.wildPlantRegrowDays.ToString("F0")), new TableDataGetter<BiomeDef>("wildPlantsCareAboutLocalFertility", (BiomeDef d) => d.wildPlantsCareAboutLocalFertility.ToStringCheckBlank()));
		}
