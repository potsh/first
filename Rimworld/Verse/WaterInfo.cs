	...
	public class WaterInfo : MapComponent
	{
		...
		public void SetTextures()
		{
			...
			if (riverOffsetTexture == null && riverOffsetMap != null && riverOffsetMap.Length > 0)
			{
				...
				riverOffsetTexture = new Texture2D(width, size2.z + 4, TextureFormat.RGFloat, mipmap: false);
		...
		public void GenerateRiverFlowMap()
		{
			if (riverOffsetMap != null)
			{
				...
				int width = size.x + 4;
				IntVec3 size2 = map.Size;
				riverFlowMapBounds = new CellRect(-2, -2, width, size2.z + 4);
				...
				{
					int newZ = (i != riverFlowMapBounds.minZ) ? (i - 1) : i;
					int newZ2 = (i != riverFlowMapBounds.maxZ) ? (i + 1) : i;
					float num = (float)((i == riverFlowMapBounds.minZ || i == riverFlowMapBounds.maxZ) ? 1 : 2);
					for (int j = riverFlowMapBounds.minX; j <= riverFlowMapBounds.maxX; j++)
					{
						int newX = (j != riverFlowMapBounds.minX) ? (j - 1) : j;
						int newX2 = (j != riverFlowMapBounds.maxX) ? (j + 1) : j;
						float num2 = (float)((j == riverFlowMapBounds.minX || j == riverFlowMapBounds.maxX) ? 1 : 2);
