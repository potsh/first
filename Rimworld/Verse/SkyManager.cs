...
namespace Verse
{
	public class SkyManager
	{
		...
		public void SkyManagerUpdate()
		{
			...
			if (map == Find.CurrentMap)
			{
				...
				else
				{
					...
					SetSunShadowVector(lightSourceInfo.vector);
					color2 = Color.Lerp(Color.white, color2, GenCelestial.CurShadowStrength(map));
				}
				GenCelestial.LightInfo lightSourceInfo2 = GenCelestial.GetLightSourceInfo(map, GenCelestial.LightType.LightingSun);
				GenCelestial.LightInfo lightSourceInfo3 = GenCelestial.GetLightSourceInfo(map, GenCelestial.LightType.LightingMoon);
		...
		private SkyTarget CurrentSkyTarget()
		{
			...
			SkyTarget a = map.weatherManager.lastWeather.Worker.CurSkyTarget(map);
			SkyTarget skyTarget = SkyTarget.Lerp(a, b, map.weatherManager.TransitionLerpFactor);
