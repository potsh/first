	...
	public class Command_SetPlantToGrow : Command
	{
		...
		public override void ProcessInput(Event ev)
		{
			...
			for (int i = 0; i < tmpAvailablePlants.Count; i++)
			{
				...
				{
					string text2 = text;
					text = text2 + " (" + "MinSkill".Translate() + ": " + plantDef.plant.sowMinSkill + ")";
				}
				...
				}, MenuOptionPriority.Default, null, null, 29f, (Rect rect) => Widgets.InfoCardButton(rect.x + 5f, rect.y + (rect.height - 24f) / 2f, plantDef)));
			}
		...
		private void WarnAsAppropriate(ThingDef plantDef)
		{
			...
			if (plantDef.plant.cavePlant)
			{
				...
				if (cell.IsValid)
				{
					Messages.Message("MessageWarningCavePlantsExposedToLight".Translate(plantDef.LabelCap).CapitalizeFirst(), new TargetInfo(cell, settable.Map), MessageTypeDefOf.RejectInput);
				}
			}
		}
