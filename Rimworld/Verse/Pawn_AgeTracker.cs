	...
	public class Pawn_AgeTracker : IExposable
	{
		...
		public string AgeNumberString
		{
			get
			{
				...
				{
					string text2 = text;
					text = text2 + " (" + AgeChronologicalYears + ")";
				}
				return text;
			}
		}
		...
		public string AgeTooltipString
		{
			get
			{
				...
				long numTicks = GenTicks.TicksAbs - birthAbsTicksInt;
				numTicks.TicksToPeriod(out int years2, out int quadrums2, out int days2, out hoursFloat);
		...
		public void AgeTick()
		{
			...
			if (ageBiologicalTicksInt % 3600000 == 0)
			{
				BirthdayBiological();
			}
		}
		...
		private void RecalculateLifeStageIndex()
		{
			...
			bool flag = cachedLifeStageIndex != num;
			cachedLifeStageIndex = num;
			if (flag && !pawn.RaceProps.Humanlike)
			{
			...
			{
				float num3 = lifeStageAges[cachedLifeStageIndex + 1].minAge - AgeBiologicalYearsFloat;
				int num4 = (Current.ProgramState == ProgramState.Playing) ? Find.TickManager.TicksGame : 0;
				nextLifeStageChangeTick = num4 + (long)Mathf.Ceil(num3 * 3600000f);
			}
			else
			{
				nextLifeStageChangeTick = 9223372036854775807L;
			}
		}
		...
		private void BirthdayBiological()
		{
			...
			{
				string text = "BirthdayBiologicalAgeInjuries".Translate(pawn, AgeBiologicalYears, stringBuilder).AdjustedFor(pawn);
				Find.LetterStack.ReceiveLetter("LetterLabelBirthday".Translate(), text, LetterDefOf.NegativeEvent, (TargetInfo)pawn);
			}
		}
