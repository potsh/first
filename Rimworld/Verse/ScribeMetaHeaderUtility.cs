	...
	public class ScribeMetaHeaderUtility
	{
		...
		public static bool TryCreateDialogsForVersionMismatchWarnings(Action confirmedAction)
		{
			...
			if (!BackCompatibility.IsSaveCompatibleWith(loadedGameVersion) && !VersionsMatch())
			{
				...
				string value = (!loadedGameVersion.NullOrEmpty()) ? loadedGameVersion : ("(" + "UnknownLower".Translate() + ")");
				text = ((lastMode == ScribeHeaderMode.Map) ? "SaveGameIncompatibleWarningText".Translate(value, VersionControl.CurrentVersionString) : ((lastMode != ScribeHeaderMode.World) ? "FileIncompatibleWarning".Translate(value, VersionControl.CurrentVersionString) : "WorldFileVersionMismatch".Translate(value, VersionControl.CurrentVersionString)));
			}
			bool flag = false;
			...
			if (text != null)
			{
				string text4 = text;
				string title = text2;
				Dialog_MessageBox dialog = Dialog_MessageBox.CreateConfirmation(text4, confirmedAction, destructive: false, title);
