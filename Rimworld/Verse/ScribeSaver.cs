	...
	public class ScribeSaver
	{
		...
		public void WriteElement(string elementName, string value)
		{
			...
			else
			{
				...
				{
					string text = curPath + "/" + elementName;
					if (!savedNodes.Add(text))
					{
						Log.Warning("Trying to save 2 XML nodes with the same name \"" + elementName + "\" path=\"" + text + "\"");
					}
				}
				try
				{
		...
		public bool EnterNode(string nodeName)
		{
			...
			if (UnityData.isDebugBuild)
			{
				...
				{
					curPath = curPath + "_" + nextListElementTemporaryId;
					nextListElementTemporaryId++;
				}
			}
			return true;
		}
		...
		public void ExitNode()
		{
			if (writer != null)
			{
				...
				if (UnityData.isDebugBuild && curPath != null)
				{
					int num = curPath.LastIndexOf('/');
					curPath = ((num <= 0) ? null : curPath.Substring(0, num));
				}
			}
		}
