...
namespace Verse
{
	public class FloatMenu : Window
	{
		...
		protected override void SetInitialSizeAndPosition()
		{
			...
			float x2 = vector.x;
			float y2 = vector.y;
			Vector2 initialSize5 = InitialSize;
			float x3 = initialSize5.x;
			Vector2 initialSize6 = InitialSize;
			windowRect = new Rect(x2, y2, x3, initialSize6.y);
		}
		...
		public override void ExtraOnGUI()
		{
			...
			if (!title.NullOrEmpty())
			{
				...
				Text.Font = GameFont.Small;
				Vector2 vector2 = Text.CalcSize(title);
				float width = Mathf.Max(150f, 15f + vector2.x);
				float x = vector.x;
				Vector2 titleOffset = TitleOffset;
				float x2 = x + titleOffset.x;
				float y = vector.y;
				Vector2 titleOffset2 = TitleOffset;
