	...
	public static class SimpleCurveDrawer
	{
			...
			if (curve.curve != null && curve.curve.PointsCount != 0)
			{
				...
				if (drawPoints)
				{
					for (int j = 0; j < curve.curve.PointsCount; j++)
					{
						...
						Vector2 screenPoint = CurveToScreenCoordsInsideScreenRect(rect, viewRect, curvePoint3.Loc);
				...
				foreach (float debugInputValue in curve.curve.View.DebugInputValues)
				{
					...
					DrawInfiniteVerticalLine(rect, viewRect, num6);
					float y = curve.curve.Evaluate(num6);
					Vector2 screenPoint2 = CurveToScreenCoordsInsideScreenRect(curvePoint: new Vector2(num6, y), rect: rect, viewRect: viewRect);
					GUI.color = Color.green;
					DrawPoint(screenPoint2);
					GUI.color = Color.white;
				}
				GUI.EndGroup();
			}
		...
		public static void DrawCurveMeasures(Rect rect, Rect viewRect, Rect graphRect, int xLabelsCount, int yLabelsCount, bool xIntegersOnly, bool yIntegersOnly)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					b = text;
					Vector2 vector = CurveToScreenCoordsInsideScreenRect(graphRect, viewRect, new Vector2(x, 0f));
					float x2 = vector.x;
					float num = x2 + 60f;
			...
			for (int j = 0; j < count2; j++)
			{
				...
				{
					b2 = text2;
					Vector2 vector2 = CurveToScreenCoordsInsideScreenRect(graphRect, viewRect, new Vector2(0f, y));
					float y2 = vector2.y;
					float num3 = y2 + (graphRect.y - rect.y);
					GUI.color = color;
		...
		private static void DrawPoint(Vector2 screenPoint)
		{
			...
			GUI.DrawTexture(position, CurvePoint);
		}

		private static void DrawInfiniteVerticalLine(Rect rect, Rect viewRect, float curveX)
		{
			Vector2 vector = CurveToScreenCoordsInsideScreenRect(rect, viewRect, new Vector2(curveX, 0f));
