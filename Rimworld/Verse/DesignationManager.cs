...
namespace Verse
{
	public sealed class DesignationManager : IExposable
	{
		...
		public void AddDesignation(Designation newDes)
		{
			...
			else if (newDes.def.targetType == TargetType.Thing && DesignationOn(newDes.target.Thing, newDes.def) != null)
			{
		...
		public IEnumerable<Designation> AllDesignationsAt(IntVec3 c)
		{
			...
			while (true)
			{
				...
				if ((!des.target.HasThing || des.target.Thing.Map == map) && des.target.Cell == c)
				{
					break;
				}
				i++;
