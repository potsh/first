...
namespace Verse
{
	public class Battle : IExposable, ILoadReferenceable
	{
		...
		public int LastEntryTimestamp => (entries.Count > 0) ? entries[entries.Count - 1].Timestamp : 0;
		...
		public static Battle Create()
		{
			...
			battle.loadID = Find.UniqueIDsManager.GetNextBattleID();
			battle.creationTimestamp = Find.TickManager.TicksGame;
			return battle;
		}
		...
		public string GetName()
		{
			...
			{
				HashSet<Faction> hashSet = new HashSet<Faction>(from p in concerns
				select p.Faction);
